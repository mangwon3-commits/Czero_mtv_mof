...
# [2026-08-28] 여기에 사용자 이름이 박혀 있었습니다 — 랩탑이 발견했습니다.
#
#   P=/home/mangwon1/mof_project/21_ZIF69_MTV
#
# 사용자가 mangwon1 이 아닌 기기에서는 이 경로가 없고, **3절이 결과 파일을
# 전부 "⬜ 아직" 으로 보고했습니다.** 파일은 내내 있었습니다.
#
# 이 유형이 세 번째입니다:
#   08-19  relax_fixcell.py 가 xtb 경로를 박아 랩탑에서 0단계로 죽음
#   08-28  wsl_hold.ps1 이 홀드 경로를 박아 **남의 계산을 죽일 뻔함**
#   08-28  이것
#
# ⚠️ **이번 것이 셋 중 제일 안 보입니다.** 1절(ps 기반)은 멀쩡해서 스크립트가
# 도는 것처럼 보이고, 틀리는 방향이 **"끝난 것을 안 끝난 것으로"** 입니다.
# §0 이 경계하는 "실패가 결과처럼 보이는 것" 의 **거울상** — 결과가 실패처럼
# 보입니다. 그리고 CLAUDE.md 가 새 세션마다 이걸 돌리라고 합니다.
#
# 그래서 유도하고, **없으면 조용히 빈칸이 아니라 죽습니다.**
P="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/21_ZIF69_MTV"
W="$HOME/.claude_work"
[ -d "$P" ] || { echo "bgstate: 프로젝트 경로가 없습니다: $P" >&2; exit 1; }
