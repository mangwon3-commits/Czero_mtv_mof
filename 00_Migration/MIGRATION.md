...
**ASE 의 `get_all_distances(mic=True)` 가 특히 위험합니다.** ZIF-69 처럼 skew 가 큰
삼사정계 셀에서는 주기 이미지를 대량 전개합니다. 구조 여러 개를 **한 프로세스에서
연달아** 감사하면 누적돼 터집니다. 구조 감사 배치가 조용히 `exit 1` 로 죽으면
이걸 의심하세요 — 개별 실행은 프로세스가 끝날 때 메모리가 반환되므로 멀쩡합니다.

기준: **원자 수가 2배면 워커도 절반으로.** `21_ZIF69_MTV/run_water.py` 는 6 으로
낮춰 두었습니다.
