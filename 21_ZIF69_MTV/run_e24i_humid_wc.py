"""MAGI-005 E-24i 습윤 TSA 작업 용량 3씨앗(차단 없음) — 대상 = env E24I_HWC_TAG. 등록 `ASSIGN_MAGI5B_20260925.md` §HKHOME 22차 · 습윤 WC 보완.
Junseok `run_e24g_br_humid_wc.py`(372c09f8 계열) 사본 — 바꾼 것: 대상(env) · 사슬 폴더(env E24I_CHAIN, 기본 ~/.mof_chain) · 잠금 · 요약 이름 · 기록 문구 · machine(env E24I_MACHINE). 원판 머리말:
...
BASE_TAG = os.environ['E24I_HWC_TAG']
STAGGER = 15.0
NOT_FOUND = "'Blocking-pocket' file not found"
CHAIN = os.environ.get('E24I_CHAIN', os.path.expanduser('~/.mof_chain')); os.makedirs(CHAIN, exist_ok=True)
LOCK = os.path.join(CHAIN, f'.{BASE_TAG}_hwc_start.lock')
SMOKE = bool(os.environ.get('E24F_SMOKE'))
MACHINE = os.environ.get('E24I_MACHINE', 'hkhome')
SUMMARY = os.path.join(HERE, f'results_e24i_{BASE_TAG}_humid_summary_{MACHINE}.json')
if SMOKE:
...
def summarize(rows, aud):
    ...
    out = {'test': f'MAGI-005 E-24i 습윤 TSA 작업 용량(차단 없음) — {BASE_TAG}', 'assign': 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차 습윤 WC 보완',
           'ported_from': 'run_e24f_humid_wc.py (E-24f, 막음 삽입 뺌)',
           'machine': MACHINE, 'made': time.strftime('%F %T'), 'smoke': SMOKE, 'runner_result': hw.RESULT,
           'blocking': None,
