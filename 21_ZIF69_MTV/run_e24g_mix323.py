# -*- coding: utf-8 -*-
"""MAGI-005 E-24g 후속 — −Br 100 의 작동점 S_mix 를 323 K 에서(E-27b 와 같은 자, 서술). 등록 `ASSIGN_MAGI5B_20260925.md` §HKHOME 21차 후속 보완 2.
`run_e27b_mix323.py` 사본 — 바꾼 것: 대상 br 추가(막음 없음 — Br 은 1.65 · 1.82 주머니 0) · 폴더 · 출력 · 문구. 원판 머리말:
...
SET = os.environ.get('E27B_SET', 'br')
M.TEMP = 323.0
M.MACHINE = os.environ.get('E27B_MACHINE', 'hkhome')
M.WORKERS = int(os.environ.get('E27B_WORKERS', '3'))
M.RUNS = os.path.join(HERE, f'e24g_mix323_runs_{SET}')
M.OUT = os.path.join(HERE, f'results_e24g_mix323_{SET}_{M.MACHINE}.json')
M.TEST = f'MAGI-005 E-24g 후속 작동점 S_mix 323 K ({SET})'
M.ASSIGN_REF = 'ASSIGN_MAGI5B_20260925.md §HKHOME 21차 후속 보완 2'
M.REG_REF = 'ASSIGN_MAGI5B_20260925.md §HKHOME 21차 후속 보완 2(서술)'
TAG = {'cl': 'e24c_cl_100', 'c2h5': 'e24c_c2h5_100', 'br': 'e24g_br_100'}[SET]
...
SH = {'cl': (128.33, 2.90, '298 K E-24c Widom ON(results_magi5_e3_e24c_cl_100_widom_hkhome.json) — 온도 다름 · 비 인용 금지'),
      'c2h5': (188.56, 4.27, '298 K E-24c 막음 Widom ON(Junseok results_e24c_och3_blockpockets_junseok.json) — 온도 다름 · 비 인용 금지'),
      'br': (144.51, 3.04, '298 K E-24g Widom ON(results_magi5_e3_e24g_br_100_widom_hkhome.json) — 온도 다름 · 비 인용 금지')}[SET]
