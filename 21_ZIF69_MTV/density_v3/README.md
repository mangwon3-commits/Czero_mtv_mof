...
데스크탑 로컬과 **랩탑(08-18 판, `…20260818_laptop.vtk` 로 이름을 갈라 둠)** 에 있습니다 — "여기에만 있다" 고 적혀 있던 것은 사실과 달랐습니다(09-06 정정) — 필요하면 요청하십시오.
...
자리를 지목하는 주장(T-4 류)에 단일 복셀 통계를 쓰지 마십시오.
**[09-06 04:1x 닫힘] 두 실행은 같은 구조(CIF md5 동일)·같은 설정(5000/2000, 298 K, 15000 Pa, 12 Å, 2×2×2)이고 씨앗만 다릅니다 → 차이는 실행 잡음입니다.**
08-18 설정은 그 실행의 RASPA 출력에서 판독했습니다(출력은 덮이지 않음). 랩탑의 구판은 `COMDensityProfile_CO2.20260818_laptop.vtk` / `DensityProfile_CO2.20260818_laptop.vtk` 로 갈라 두었습니다.
