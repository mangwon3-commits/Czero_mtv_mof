# -*- coding: utf-8 -*-
"""MAGI-005 E-28e — E-24i 열린 자리 치환체 습윤(RH90) 밀도 격자. `run_e28c_density.py`(Junseok E-28c) 사본 — 바꾼 것: 대상 = 명령줄 인자 · 막음 파일 = e24i_zeo_runs/<대상>/block{1.65,1.30}/ · 감사 파일 이름 e28e_audit.json. 원판 머리말:
...
끝에서(래퍼): 관문 감사(완주 · 씨앗 · C₂H₅ 는 성분마다 blocked 줄과 N = 구 × 단위셀) → <DW_SUB>/e28e_audit.json · COM 격자 VTK gzip(E-28 과 같이
...
BASE_TAG = sys.argv[1] if len(sys.argv) > 1 else ''
    ...
    def _run(cmd, *a, **k):
        ...
        if cwd and cmd and str(cmd[-1]) == 'simulation.input':
            ...
            try:
                ...
                ef.close()
            with open(os.path.join(cwd, 'e28c_returncode.txt'), 'w') as f:
...
def post(target, rc):
    ...
    if len(outs) == 1:
        ...
        if BLOCKING:
            n = uc[0] * uc[1] * uc[2]
            ba = block_audit(txt)
            rcf = os.path.join(d, 'e28c_returncode.txt')
    ...
    a['com_grids_gz'] = gz
    json.dump(a, open(os.path.join(D.OUT, 'e28e_audit.json'), 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    print(f'E-28c 밀도 격자 — 대상 {sys.argv[1]} · 막음 {BLOCKING} · 결과 {D.OUT} · 작업 {rw.RUNS}', flush=True)
    rc = D.main()
    post(sys.argv[1], rc)
    sys.exit(rc)
