# -*- coding: utf-8 -*-
"""E-22d — 꼭짓점 B(e22d_coregeom) · D(e22d_relaxgeom_coreq) 물 Widom 4씨앗. 등록 ASSIGN_MAGI5B §HKHOME 9차.
...
rw.RUNS = os.path.join(HERE, 'e22d_runs')
OUT = os.path.join(HERE, 'results_e22d_water_hkhome.json')
NAMES = ('e22d_coregeom', 'e22d_relaxgeom_coreq'); NSEED = 4; WORKERS = 4
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=WORKERS) as ex:
        for fu in as_completed([ex.submit(rw.one, j) for j in jobs]):
            ...
            json.dump({'test': 'E-22d', 'finished': False, 'ff_md5': m, 'water_def_md5': wm, 'rows': rows}, open(OUT, 'w'), indent=1)
    seeds = [r.get('seed') for r in rows]
    json.dump({'test': 'E-22d', 'finished': True, 'ff_md5': m, 'water_def_md5': wm, 'seed_dup': len(seeds) - len(set(seeds)), 'rows': rows}, open(OUT, 'w'), indent=1)
    print('E-22d 물 완주', flush=True)
