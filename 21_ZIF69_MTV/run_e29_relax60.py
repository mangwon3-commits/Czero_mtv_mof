# -*- coding: utf-8 -*-
"""MAGI-005 E-29 (Junseok 15차) — CH₃ 의 통로 닫힘은 UFF4MOF 이완을 수렴시켜도 남는가. 등록 ASSIGN_MAGI5B §Junseok 15차(65a870ac, 자료 0건).

자: run_e24_gate5.py 와 같은 처리 — risk_screen.run_one(무수정 import · UFF4MOF · lammps_mof). 바꾸는 것:
  · rs.OUTER_LOOP_CAP = 60 (12 → 60, 래퍼에서 덮음 — risk_screen.py 수정 없음)
  · rs.STRUCT = e29_stage · rs.WORK = lmp_e29 (새 폴더 — run_one 은 min_<이름>.data 가 있으면 재사용하므로 관문 ⑤ 의 lmp_e24 를 쓰면 안 됨)
...
관문 ⑤ 결과(results_e24_gate5_junseok.json)는 건드리지 않음. 출력 results_e29_relax60_junseok.json. 기계적 사실뿐 — 판정은 종합자.
...
TAGS = ['e24_ch3_100', 'e24_cn_100', 'e22_parent']
CAP = 60
STAGE = os.path.join(HERE, 'e29_stage')
WORK = os.path.join(HERE, 'lmp_e29')
OUT = os.path.join(HERE, 'results_e29_relax60_junseok.json')
...
def classify(rows):
    reg = {}
    r = rows.get('e24_ch3_100', {})
    a = r.get('after') or {}
    pld, av = a.get('PLD'), a.get('AV_per_cell')
    if pld is not None and av is not None:
        if pld < 3.3 or av < 20:
            b = '성립 쪽(닫힘 유지: PLD < 3.3 이거나 AV/셀 < 20)'
        elif pld >= 3.64 and av > 20:
            b = '기각 쪽(PLD ≥ 3.64 이고 AV/셀 > 20)'
        else:
            b = '띠(3.3 ≤ PLD < 3.64, AV/셀 ≥ 20)'
        reg['(1)_CH3'] = {'PLD_after': pld, 'AV_per_cell_after': av, 'branch': b}
    for t in ('e24_cn_100', 'e22_parent'):
        a = (rows.get(t) or {}).get('after') or {}
        if a.get('PLD') is not None and a.get('AV_per_cell') is not None:
            reg[f'(2)_{t}'] = {'PLD_after': a['PLD'], 'AV_per_cell_after': a['AV_per_cell'], 'open': a['PLD'] >= 3.64 and a['AV_per_cell'] > 20}
    return reg
...
if __name__ == '__main__':
    print(f'E-29 — 대상 {TAGS} · OUTER_LOOP_CAP {rs.OUTER_LOOP_CAP} · INNER {rs.INNER_ITER_CAP} · STRUCT {rs.STRUCT} · WORK {rs.WORK} · '
          f'NETWORK {rs.NETWORK} · LMP {rs.LMP} · IFACE 존재 {os.path.exists(rs.IFACE)} · python {sys.executable}', flush=True)
    if os.environ.get('E29_DRY'):
    ...
    if os.path.exists(OUT) or os.path.exists(STAGE) or os.path.exists(WORK):
        print('!! 이전 E-29 산출(출력 · e29_stage · lmp_e29) 있음 — 중단', flush=True)
        sys.exit(1)
    os.makedirs(STAGE)
    os.makedirs(WORK)
    t0 = time.time()
    res = {'test': 'MAGI-005 E-29 — CH₃ 통로 닫힘 · UFF4MOF 이완 상한 60', 'assign': 'ASSIGN_MAGI5B_20260925.md §Junseok 15차', 'machine': 'junseok',
           'rule': 'risk_screen.run_one 무수정 · OUTER_LOOP_CAP 60(래퍼) · INNER 2000 · min_eval 1e-4 · Zeo++ 호출 잠금(순차)', 'cap': CAP,
