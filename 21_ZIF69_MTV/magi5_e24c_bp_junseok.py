...
BASE = os.path.join(HERE, 'e24c_bp_runs')
OUT = os.path.join(HERE, 'results_e24c_och3_blockpockets_junseok.json')
...
def block_file(tag, r):
    return os.path.join(HERE, 'e24c_zeo_runs', tag, f'block{r:.2f}', f'{tag}_relaxed.block')
...
def unblocked_ref(tag):
    ...
    key = tag.split('_')[1]                       # och3 · c2h5 — 데스크탑 · laptop2 의 --tag 이름을 몰라 치환기 이름으로 찾음
    fs = sorted(set(glob.glob(os.path.join(HERE, f'results_magi5_e3_*{key}*_widom_*.json'))))
    for f in fs:
        try:
...
def main():
    ...
    ap.add_argument('--tags', default='e24c_och3_100')
    a = ap.parse_args()
    ...
    print(f'E-24c 막음 Widom {len(jobs)}작업 · 좌표 {json.dumps(checks)} · 계획 {json.dumps(plan, ensure_ascii=False)}', flush=True)
    ...
    if n('simulate') not in ('', '0') or n('network') not in ('', '0') or n('lmp_serial') not in ('', '0'):
        print('!! simulate · network · lmp_serial 중 도는 것 있음 — 중단(Zeo++ 와 RASPA 동시 금지)', flush=True)
        return 1
    if os.path.exists(BASE) or os.path.exists(OUT):
        print('!! 이전 산출(e24c_bp_runs · 결과) 있음 — 중단', flush=True)
        return 1
    res = {}
    ...
    out = {'test': 'MAGI-005 E-24c 막음 Widom(BlockPockets) — 주머니 있는 형판 치환체, 전하 ON/OFF', 'machine': 'junseok',
           'assign': 'ASSIGN_MAGI5B_20260925.md E-24c 보완 02:14(3efe1d37)', 'zeo': 'E-24c ① -ha -block r 50000 (results_e24c_access_junseok.json)',
