...
# ===================================================================
# [01:50 등록 추가 — 완주 0/9 시점]  이질성 검사.
#
#   다섯 평균은 "delta 가 다섯에 공통" 을 가정합니다. 그 가정이 깨지면
#   평균은 여전히 뭔가의 평균이지만 **하나의 배치 편차** 로는 못 읽습니다.
#   Q = sum((drop_i - 평균)^2 / Var_i),  dof 4.  등록 문턱 Q > 9.488 (p<0.05).
# ===================================================================
if len(have) == 5:
    # [01:55 정정 — 랩탑] Q 의 중심은 **역분산 가중 평균**이어야 chi2(4) 가 됩니다.
    # 동일가중 평균은 이 이차형식의 최소점이 아니라 Q 를 부풀립니다.
    # (이 자료는 반복수 7/4/3/3/3 이라 분산비 1.167, 실제 차이는 없습니다.
    #  e4 가 들어오거나 반복수가 더 벌어지면 갈립니다.)
    # 보고 추정량은 동일가중(편향 0), Q 의 중심은 역분산(정의) — 목적이 다릅니다.
    _v = {k: R * R * (1 + 1.0 / drops[k][1]) for k in have}
    _W = sum(1.0 / _v[k] for k in have)
    _c = sum(drops[k][0] / _v[k] for k in have) / _W
    Q = sum((drops[k][0] - _c) ** 2 / _v[k] for k in have)
    print()
    print("  [이질성] Q = %.2f   dof 4   문턱 9.488   (중심 %.5f = 역분산 평균,"
          % (Q, _c))
    print("           보고 추정량 %+.5f = 동일가중과 구분)" % d5)
    if Q > 9.488:
        print("    -> **다섯이 공통 delta 로 안 묶입니다.** 위 평균을 배치 편차 하나로")
        print("       읽으면 안 됩니다 — 구조마다 다른 것이 섞여 있습니다.")
    else:
        print("    -> 공통 delta 가정과 모순 없음. 평균을 배치 편차로 읽어도 됩니다.")
        print("       (dof 4 라 검정력이 낮습니다 — 모순이 없다는 것이지 확인은 아닙니다.)")
