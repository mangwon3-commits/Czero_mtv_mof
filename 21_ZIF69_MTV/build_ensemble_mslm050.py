"""자리 배치 앙상블 — mslm050(−SO₂CH₃ 12/24) 을 시드 1~5 로 만든다. T-SA-1 짝 설계(TSA1_REGISTRATION_20260911.md): 같은 시드·같은 WANT=12 이면 saIm050e{k} 와 같은 12자리가 뽑히므로 mslm050e{k} 는 saIm050e{k} 의 짝이다(치환기만 다름). 원본: build_ensemble_mslm025.py(09-09).

[왜]
    지금까지의 모든 v3 결과는 seed=0 의 단일 실현이다. 같은 14/24 조성에서
    어느 14자리를 고르느냐가 흡착을 얼마나 흔드는지 잰 적이 없다
    (ENSEMBLE_0500_PROTOCOL.md — 판정 기준은 계산 전에 등록돼 있다).
...
[태그]
    saIm050e{seed} — e 뒤 숫자가 빌더 시드다. seed=0(생산)은 만들지 않는다.
...
# 648 = 600 + 4*12. Cl+S = 24 가 자리 보존의 독립 검산이다.
# 실제 saIm050 구조에서 읽어 확인한 값이다(손으로 유도하지 않음).
EXPECT_EL = {"Zn": 24, "C": 252, "N": 120, "O": 72, "Cl": 12, "S": 12, "H": 180}   # structures_v2/ZIF69_mslm050.cif 실측 (09-11, ase)
...
def main():
    ap = argparse.ArgumentParser(description="mslm050 자리 배치 앙상블 생성 — T-SA-1 짝 설계(시드 1~5 = saIm050e1~e5 와 같은 배열), TSA1_REGISTRATION_20260911")
    ...
    for seed in range(1, MAX_SEED):
        ...
        if os.path.exists(out_cif) and not a.force:
            ...
            taken.append(tag)
            continue
        comp = {"clIm_aryl": 1.0 - FRAC, "mslm_aryl": FRAC}
        try:
        ...
        if not ok:
            ...
            continue
        taken.append(tag)
        rows.append({"tag": tag, "sub": "mslm_aryl", "group": "-SO2CH3",
                     ...
                     "note": "09-11 mslm050 자리 배치 앙상블 — T-SA-1 짝 설계: 시드 1~5 = saIm050e1~e5 와 같은 12자리 배열(공여 H 유무만 다름). e 뒤 숫자 = 빌더 시드. "
                             "ENSEMBLE_0500_PROTOCOL.md 참조"})
