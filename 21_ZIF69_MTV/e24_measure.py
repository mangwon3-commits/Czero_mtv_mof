"""E-24 들어가는가 관문(등록 §HKHOME 13차 (0)) — E-22a 와 같은 양: ① 비결합 최소 접촉(위상거리 ≥ 3, 이종 원자–H 포함) ② 치환기 아닌 골격 원자 최대 변위(같은 원소 최근접 짝, 순서 무관).
...
TAGS = {'e24_ch3_100': 'E24_ZnDia_CH3_100', 'e24_f_100': 'E24_ZnDia_F_100', 'e24_cn_100': 'E24_ZnDia_CN_100'}
HEAVY_H = ('H···O', 'H···N', 'F···H')
...
for tag, cif in TAGS.items():
    a0 = read(f'e24_candidates/{cif}.cif'); a1 = read(f'relax_tnf/{tag}_relaxed.cif'); assert len(a0) == len(a1)
    ...
    het_h = min([c1[p][0] for p in c1 if p in HEAVY_H] or [9.9])
...
json.dump(res, open('results_e24_fit_hkhome.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
