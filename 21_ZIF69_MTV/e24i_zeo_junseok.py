# -*- coding: utf-8 -*-
"""MAGI-005 E-24i (Junseok) — 열린 자리(고리 2 · 3) 치환체 다섯의 접근성: Zeo++ `-ha -res` · `-ha -chan r` · `-ha -block r 50000`. 등록 ASSIGN_MAGI5B §HKHOME 22차(f5935fdf).
e24g_zeo_junseok.py(E-24g 에 쓴 것) 사본 — 바꾼 것: 실행 폴더 e24i_zeo_runs · 출력 results_e24i_access_junseok.json · 기록 문구(test · assign · machine=junseok) ·
  **동시 2건**(E24I_ZEO_WORKERS, 기본 2 — 데스크탑 허용 "동시 2건까지"; 대상마다 한 워커, 한 대상 안의 7회는 순차) — 대상 루프 본문을 함수 one() 으로 옮김.
  측정(명령 · 반경 1.65 · 1.82 · 1.30 · 표본 50000 · 파싱 · RSS 가드 5000 MB · 요약)은 그대로. 원판 머리말:
MAGI-005 E-24g · E-24h (Junseok 실행 사본 — e24g_zeo_desktop.py 에서 경로 · 출력 이름만 바꿈) — 새 치환체의 접근성: Zeo++ `-ha -res` · `-ha -chan r` · `-ha -block r 50000`.
...
출력 results_e24g_access_junseok.json · 실행 폴더 e24g_zeo_runs/. 기계적 사실뿐 — 판정은 판정문에서.
"""
import glob
import json
import math
...
BASE = os.path.join(D, 'e24i_zeo_runs')
OUT = os.path.join(D, 'results_e24i_access_junseok.json')
NS = 50000
RADII = (1.65, 1.82, 1.30)
NAMES = tuple(sys.argv[1].split(',')) if len(sys.argv) > 1 else ()
PLD_MIN = 3.64
GUARD_MB = 5000
WORKERS = int(os.environ.get('E24I_ZEO_WORKERS', '2'))
...
def main():
    if not NAMES:
        print('사용: e24i_zeo_junseok.py tag1,tag2,...', flush=True)
        return 1
    if busy():
    ...
    out = {'test': 'MAGI-005 E-24i — 열린 자리 치환체 접근성(Zeo++)', 'assign': 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차', 'machine': 'junseok', 'workers': WORKERS,
           'ported_from': 'e24g_zeo_junseok.py (E-24g) ← junseok magi5_e24b_zeo_junseok.py (255cf19e)',
