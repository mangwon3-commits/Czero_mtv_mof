"""안정성 관문 자기 산포 — `base` + `nbIm050` 앙상블 (e1~e5). ASSIGN_NB050_20260906 §4. risk_screen_v3ens0500.py 코드부 복제.
출력: risk_results_v3ens_nb050.json  대상: ENS_NB050_TAGS 또는 relax_v3/ZIF69_nbIm050*_relaxed.cif
"""
import os as _os
...
STAGE = os.path.join(HERE, "structures_v3ens_nb050_stage")
...
def _discover():
    env = os.environ.get("ENS_NB050_TAGS", "").strip()
    if env:
...
sys.argv = ["risk_screen.py", "risk_v3ens_nb050_index.json", "v3ens_nb050"]
...
def main():
    ...
    assert rs.STRUCT.endswith("structures_v3ens_nb050_stage"), "경로가 안 잡혔습니다"

    idx = os.path.join(HERE, "risk_v3ens_nb050_index.json")
