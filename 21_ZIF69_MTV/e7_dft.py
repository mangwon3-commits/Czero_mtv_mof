...
def cp_bind(frag_sym, frag_xyz, wat_xyz, basis, charge=0):
    sym = list(frag_sym) + ['O', 'H', 'H']; xyz = list(frag_xyz) + list(wat_xyz)
    nf = len(frag_sym); wi = set(range(nf, nf + 3)); fi = set(range(nf))
    eab, c1 = energy(sym, xyz, set(), basis, charge)
    ea, c2 = energy(sym, xyz, wi, basis, charge)       # 조각 + 물 유령
    eb, c3 = energy(sym, xyz, fi, basis, 0)            # 물 + 조각 유령
