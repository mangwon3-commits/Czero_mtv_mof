# E-24j (HKHOME 24차) — run_e24i_relax.py 사본: 폴더 e24j_stage · lmp_e24j · 출력 results_e24j_relax300_<기기>.json 만 바꿈. 대상 e22_parent(모체 대조).
# -*- coding: utf-8 -*-
"""MAGI-005 E-24i (HKHOME 22차) — `run_e24g_relax.py` 사본(폴더 · 출력 · 문구만). 원판: MAGI-005 E-24g · E-24h (HKHOME 21차) — 새 치환체 · 희석 실현의 UFF4MOF 셀 이완(상한 300, 수렴 EDiff < 1e-4). 등록 ASSIGN_MAGI5B §HKHOME 21차(자료 0건).
`run_e29b_relax.py`(E-29b) 사본 — 바꾼 것: 대상(명령줄 쉼표 목록) · 폴더 e24g_stage · lmp_e24g · 출력 · classify(등록 (4c) 의 "열림" = PLD ≥ 3.64 · AV > 0, 모든 대상에 서술). 그 원문 첫 줄:
MAGI-005 E-29b (HKHOME 18차) — CH₃ · Cl 의 UFF4MOF 닫힘은 수렴에서도 남는가.
...
TAGS = [t for t in (sys.argv[1].split(',') if len(sys.argv) > 1 else []) if t]
CAP = 300
STAGE = os.path.join(HERE, 'e24j_stage')
WORK = os.path.join(HERE, 'lmp_e24j')
OUT = os.path.join(HERE, f"results_e24j_relax300_{os.environ.get('E24I_MACHINE', 'hkhome')}.json")
...
def classify(rows):
    """등록 (4c) 의 "열림" = 처리 뒤 PLD ≥ 3.64 Å 이고 CO₂ 탐침 AV/셀 > 0 · 수렴 = 최종 EDiff < 1e-4. 모든 대상에 같은 식(100 % 는 서술)."""
    reg = {}
    for t, r in rows.items():
        ...
        reg[t] = {'PLD_after': pld, 'AV_per_cell_after': av, 'open': bool(pld >= 3.64 and av > 0),
                  'channels_after_1.65': (r.get('chan_after') or {}).get('n_channels'),
                  'final_EDiff': ed, 'outer_loops': r.get('outer_loops'), 'converged': (ed is not None and ed < 1e-4)}
    return reg
...
if __name__ == '__main__':
    if not TAGS:
        print('사용: run_e24g_relax.py tag1,tag2,...', flush=True); sys.exit(2)
    print(f'E-24g UFF4MOF — 대상 {TAGS} · OUTER_LOOP_CAP {rs.OUTER_LOOP_CAP} · INNER {rs.INNER_ITER_CAP} · STRUCT {rs.STRUCT} · WORK {rs.WORK} · '
          f'NETWORK {rs.NETWORK} · LMP {rs.LMP} · IFACE 존재 {os.path.exists(rs.IFACE)} · python {sys.executable}', flush=True)
    if os.environ.get('E24G_DRY'):
    ...
    if os.path.exists(OUT) or os.path.exists(STAGE) or os.path.exists(WORK):
        print('!! 이전 E-24g 산출(출력 · e24g_stage · lmp_e24g) 있음 — 중단', flush=True)
        sys.exit(1)
    os.makedirs(STAGE)
    os.makedirs(WORK)
    t0 = time.time()
    res = {'test': 'MAGI-005 E-24i — UFF4MOF 이완 수렴(상한 300)', 'assign': 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차', 'machine': os.environ.get('E24I_MACHINE', 'hkhome'),
