"""자리 배치 앙상블 — saIm025 를 다른 시드 5개로 다시 만든다.

[왜]
    지금까지의 모든 v3 결과는 seed=0 의 단일 실현이다. 같은 6/24 조성에서
    어느 6자리를 고르느냐가 흡착을 얼마나 흔드는지 잰 적이 없다
    (ENSEMBLE_0500_PROTOCOL.md — 판정 기준은 계산 전에 등록돼 있다).
    saIm025 만 앙상블이 없어 MAGI-004 §6-5 가 "−SO3H 25~58 %" 문장을
    050·58 % 로 축소해야 했다. 이 빌더가 그 구멍을 메운다
    (SAIM025E_HUMID_WC_REGISTRATION_20260917.md §1).
...
[태그]
    saIm025e{seed} — e 뒤 숫자가 빌더 시드다. seed=0(생산)은 만들지 않는다.
    시드 1부터 올려가며 검사 통과분 5개를 채운다.

사용:
    python3 build_ensemble_025.py --dry-run
    python3 build_ensemble_025.py
"""
import argparse
import json
import os
...
# 624 = 600 + 4*6. Cl+S = 24 가 자리 보존의 독립 검산이다.
# 실제 saIm025 생산 구조(structures_v2/ZIF69_saIm025.cif, seed=0)를 ase 로
# 읽어 확인한 값이다(2026-09-17, 손으로 유도하지 않음).
EXPECT_EL = {"Zn": 24, "C": 240, "N": 120, "O": 66, "Cl": 18, "S": 6, "H": 150}
...
def main():
    ap = argparse.ArgumentParser(description="saIm025 자리 배치 앙상블 생성")
    ...
    for seed in range(1, MAX_SEED):
        ...
        rows.append({"tag": tag, "sub": "saIm_aryl", "group": "-SO3H",
                     ...
                     "note": "2026-09-17 자리 배치 앙상블(saIm025). e 뒤 "
                             "숫자 = 빌더 시드. ENSEMBLE_0500_PROTOCOL.md 와 "
                             "SAIM025E_HUMID_WC_REGISTRATION_20260917.md 참조"})
