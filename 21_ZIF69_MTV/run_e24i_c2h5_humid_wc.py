# -*- coding: utf-8 -*-
"""MAGI-005 E-24i 습윤 WC 보완 (Junseok) — −C₂H₅ 열린 자리(e24i_c2h5_open) 습윤 TSA 작업 용량 3씨앗, **막음**(E-24f 방식). 등록 ASSIGN_MAGI5B §HKHOME 22차 습윤 WC 보완(8bbec7bf).
데스크탑 부탁: "run_e24f_humid_wc.py 틀에서 막음 파일만 e24i_zeo_runs/e24i_c2h5_open/block{1.65,1.30}/ 로".
run_e24f_humid_wc.py(Junseok E-24f) 사본 — 바꾼 것: 대상(BASE_TAG, 바이트 같은 사본 _h1~_h3) · 막음 파일(CO₂ = E-24i block1.65 구 4 · 물 = block1.30 구 5) ·
  출력 이름(요약 results_e24i_e24i_c2h5_open_humid_summary_junseok.json · rc 파일 · 잠금 · 스모크 폴더) · E-24f 등록 (1) 의 CH₃ 대비 계산 뺌(이 등록엔 없음) · 문구.
  자(run_humid_wc_v3w → run_humid_wc 무수정) · 성분별 막음 삽입 · 관문 · 착수 간격(15 s + 2 s 잠금)은 원판 그대로. 원판 머리말:
...
BASE_TAG = 'e24i_c2h5_open'
BLOCK = {'CO2': os.path.join(HERE, 'e24i_zeo_runs', BASE_TAG, 'block1.65', BASE_TAG + '_relaxed.block'),
         'water': os.path.join(HERE, 'e24i_zeo_runs', BASE_TAG, 'block1.30', BASE_TAG + '_relaxed.block')}
KEY = {'CO2': 'co2', 'water': 'water'}
STAGGER = 15.0
NOT_FOUND = "'Blocking-pocket' file not found"
CHAIN = '/home/mangwon/.junseok_chain'
LOCK = os.path.join(CHAIN, '.e24i_c2h5_hwc_start.lock')
SMOKE = bool(os.environ.get('E24F_SMOKE'))
SUMMARY = os.path.join(HERE, f'results_e24i_{BASE_TAG}_humid_summary_junseok.json')
if SMOKE:
    hw.CYCLES, hw.INIT = 10, 0
    hw.TARGETS = [BASE_TAG + '_h1']
    hw.RUNS = os.path.join(CHAIN, 'e24i_c2h5_smoke_runs')
    hw.RESULT = os.path.join(CHAIN, 'e24i_c2h5_smoke_result.json')
    SUMMARY = os.path.join(CHAIN, 'e24i_c2h5_smoke_summary.json')
...
def _run(args, *a, **k):
    ...
    if cwd and args and args[0] == rg.SIMULATE:
        ...
        try:
            ...
            ef.close()
        with open(os.path.join(cwd, 'e24i_returncode.txt'), 'w') as f:
...
def audit():
    ...
    for t in hw.TARGETS:
        for lab, *_ in hw.CONDITIONS:
            ...
            rcf = os.path.join(d, 'e24i_returncode.txt')
            ba = block_audit(txt)
...
def summarize(rows, aud):
    ...
    out = {'test': 'MAGI-005 E-24i — C₂H₅ 열린 자리(e24i_c2h5_open) 습윤 TSA 작업 용량(막음)', 'assign': 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차 습윤 WC 보완(8bbec7bf)',
           'ported_from': 'run_e24f_humid_wc.py (E-24f, 막음 파일만 E-24i 로)',
...
if __name__ == '__main__':
    ...
    for c in ('CO2', 'water'):
        if not os.path.exists(BLOCK[c]):
            ...
            sys.exit(2)
    print(f"E-24i 습윤 작업 용량(C₂H₅ 열린 자리 막음) — 착수 간격 {STAGGER:.0f} s(+ 2 s 잠금) · 작업 {len(JOBS)} · 막음 CO₂ {nsph('CO2')} 구 · 물 {nsph('water')} 구 · "
