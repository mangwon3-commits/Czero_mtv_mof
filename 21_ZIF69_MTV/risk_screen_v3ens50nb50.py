...
[★ base 를 반드시 포함합니다]
    ...
    **v1(`risk_results.json`, 7.83298)** 으로 대체합니다. v3 판정이 쓴 자는
    ...
    [2026-08-28 정정] **기대값을 못 박을 수 없습니다.**
    base 는 `angle_style` 순서에 따라 **7.61978 또는 7.63144** 를 냅니다.
    08-27·08-28 에 7.61978 이 두 번 나온 것은 그 실행들이 같은 순서를
    뽑았기 때문이고, 이 실행(08-28 14:57)은 7.63144 를 냈습니다.
    **둘 다 정상이며 `base` 값으로 이상을 판정할 수 없습니다.**
    이상 판정에 쓸 수 있는 것은 v1 대체값 **7.83298** 뿐입니다 —
    그것이 나오면 base 가 이번 실행에 안 들어간 것입니다.
