...
**방법 메모(경고 아님, 반복 확인)**: 이번 체크아웃도 얕은 클론으로
시작했다(`.git/shallow`에 `--depth 50` 경계 커밋 두 개). unshallow 전
`git rev-list --left-right --count origin/master...origin/laptop-20260822`
는 "master 50 앞섬 / laptop 2844 앞섬"을, `git merge-base`는 **공통
조상 없음**을 보고했다 — laptop 이 08-03 `Initial checkpoint`까지 이어지는
반면 얕은 master 는 50개만 갖고 있어 생긴 착시다(09-24·09-25 감사가 같은
함정을 잡았던 것과 동일 유형). `git fetch --all --unshallow` 후
재확인했고, **아래 1절 수치는 전부 unshallow 이후 값**이다.

### 1) 브랜치 정체

기준 시각 2026-09-26 00:12 UTC(= 09:12 KST). `git fetch --all` 후:

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `405b02fb` 2026-09-26T06:06:11+09:00 | 3.1시간 | — | — |
| `origin/laptop-20260822` | `2e676295` 2026-09-26T09:02:54+09:00 | 0.15시간 | **450커밋** | 526커밋 |
| `origin/junseok-20260822` | `158be022` 2026-09-26T05:14:52+09:00 | 4.0시간 | 12커밋 | 234커밋 |

(`git rev-list --left-right --count origin/master...origin/<브랜치>`, 전체
이력 기준. master 전체 3,159커밋 · laptop-20260822 전체 3,235커밋.)

**경고: `laptop-20260822` 가 master 대비 450커밋 뒤처짐 — 문턱(24) 큰 폭
초과.** `merge-base`는 `0be33d5c`(2026-09-24T21:22:59+09:00, "§-보완2
검산")이고, 마지막 동기화로부터 **약 35.8시간** 경과. laptop 은 그 사이
자기 커밋 526개(`[postman:laptop] 결과 파일 자동 반입` 다수)를 쌓았지만
master 의 새 커밋 450개를 받아오지 않았다. `git log
origin/laptop-20260822..origin/master -- CLAUDE.md
21_ZIF69_MTV/ERROR_BARS.md`는 0건 — CLAUDE.md·ERROR_BARS.md 자체는 두
판이 바이트 동일해 08-24 사고(교리 파일 "없음" 오인)와 같은 위험은 지금은
없다. 다만 450커밋 안에는 E-24f·E-28c 판정, MAGI5 결정 등 최근 이틀치
내용이 들어 있어 laptop 세션이 그 맥락 없이 작업 중이라면 낡은 전제로
움직일 수 있다.

`junseok-20260822`는 12커밋 뒤처짐으로 문턱 미만, 정상.

### 2) 우편함 침묵

`21_ZIF69_MTV/COMMS/*.md` 마지막 갱신 커밋(기준 시각 동일):

| 파일 | 마지막 커밋 | 경과 |
|---|---|---|
| `desktop.md` | 2026-09-26T04:41:25+09:00 | 4.5시간 |
| `laptop.md` | 2026-09-21T15:12:20+09:00 | 4.75일 |
| `junseok.md` | 2026-08-29T11:32:05+09:00 | 27.9일 |
| `laptop2.md` | 2026-09-10T01:13:30+09:00 | 16.3일 |
| `cloud4c.md` | 2026-09-04T17:49:13+09:00 | 21.6일 |

기기가 계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음 —
경과 시간만 보고. `laptop.md`는 4.75일째 갱신이 없지만 `laptop-20260822`
브랜치 자체는 0.15시간 전에도 커밋이 있었다(postman 결과 반입) — 우편함이
아니라 결과 파일만 오가고 있다는 뜻으로 읽힌다.

**부기 — `audit.md` 자체의 무결성 사고**: 이 파일의 히스토리를 보면
2026-09-25 09:45:51+09:00 커밋 `17fe6387`([postman:desktop] 결과 파일
자동 반입 2건)가 결과 JSON 파일 하나를 반입하면서 같은 커밋에서 이
파일의 "2026-09-25 00:14 UTC — 일일 감사" 항목(175줄)을 **통째로
삭제**했다(`git show 17fe6387 --stat`: `audit.md | 175 ---`). 현재
파일에 그 날짜 항목이 없음을 `grep`으로 확인했다. 결과 파일 반입 커밋이
무관한 COMMS 파일의 내용을 지운 것으로, 이 저장소의 반복 결함 유형과
같은 모양이다(반입 스크립트가 로컬의 낡은 사본을 기준으로 덮어썼을
가능성). **이 감사 스스로도 §0 이 경고하는 유형에 당했다** — 삭제된 09-25
항목은 이 커밋 로그에서만 복구 가능하며, 이번 항목에서는 복구하지 않고
사실만 기록한다(이 파일은 감사 전용이라 손대는 사람이 이 자동화뿐이어야
하는데, postman 자동 반입이 예외적으로 건드렸다는 뜻).

### 3) 결과 JSON 무결성

`results_v3*.json`(11개) + `v3_wc/`(2개) + `v3_humid_wc/`(7개) +
`v3_humid_wc_ens/`(3개) + `v4_humid_wc/`(2개) + `v3_water_grid/`(4개) +
`v3_water_grid_cliff/`(2개) = 총 31개 파일을 파이썬으로 파싱.

**이상 없음** — 31개 전부 파싱 성공, 0행 파일 없음(`rows` 키 또는
리스트 최상위 길이로 계수). 예: `results_v3.json` 31행,
`v3_water_grid/water_results_laptop.json` 12행, `v4_humid_wc/*` 각 1~2행.

### 4) 출처 규약 위반

`merge_water_batches.py`의 `machine_of()`는 `water_results_<기기>.json`
형식이 아니면 `SystemExit`으로 거부한다(태그 없는 파일을 글롭해 넘기면
막힘) — 이 보호 자체는 살아 있음을 코드로 확인.

그런데 **디렉터리 안에 태그 없는 사본과 태그 붙은 사본이 여전히 byte
단위로 동일하게 공존**한다:

- `v3_water_grid/water_results.json` == `v3_water_grid/water_results_desktop4.json`
  (파이썬 `==` 비교 `True`, saIm0583 4행 전부 동일)
- `v3_water_grid_cliff/water_results.json` == `v3_water_grid_cliff/water_results_desktopcliff.json`
  (`True`, saIm0917·saIm0958 4행 전부 동일)

08-23 사고와 같은 모양의 **중복**이다 — `machine_of()`가 태그 없는
파일명을 거부하므로 지금 당장 이중 계수로 이어지지는 않지만(태그 없는
파일을 실수로 넘기지 않는 한), 디스크에 이 쌍이 남아 있는 한 위험은
남는다.

반대로 **진짜 교차 검증**도 확인됨: `saIm0875`가
`water_results_junseok.json`과 `water_results_laptop.json` 양쪽에 있고,
4개 RH(0.0/0.25/0.5/0.9) 전부에서 **값이 서로 다르다**
(예: RH=0.0 CO2_molkg junseok 1.2905 vs laptop 1.3031) — 같은 조성을
두 기기가 독립적으로 계산한 것으로, 중복이 아니다.

### 5) 사전 등록 관문 대비 기록

`v3_water_grid*/` 태그 파일에서 RH0·RH90 CO2 로딩으로 유지율(RH90/RH0)을
직접 재계산:

| 조성 | 출처 | RH0 | RH90 | 유지율 | 등급 |
|---|---|---|---|---|---|
| `saIm0583` | desktop4 | 1.3188 | 1.0010 | 75.90% | 조건부(50~80%) |
| `saIm0875` | junseok | 1.2905 | 0.9619 | 74.54% | 조건부(50~80%) |
| `saIm0875` | laptop | 1.3031 | 0.9671 | 74.21% | 조건부(50~80%) |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.74% | 조건부(50~80%) |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.06% | 조건부(50~80%) |
| `saIm0917` | desktopcliff | 1.3249 | 0.9725 | 73.41% | 조건부(50~80%) |
| `saIm0958` | desktopcliff | 1.5205 | 1.0032 | 65.98% | 조건부(50~80%) |

**이상 없음** — 7개 값 전부 50~80% 구간에 있고 "유효(≥80%)"·"종료(<50%)"
경계를 넘는 곳이 없다. `COMMS/desktop.md`·`junseok.md` 등에 적힌 기존
"조건부" 판정과 어긋나는 곳을 찾지 못했다(과거 감사들의 반복 대조와
동일 결론).

### 6) WC 자료 공백

`regen_energy_v3.py`의 `load_wc()`를 직접 실행해 확인(08-27에 이미
글롭 기반으로 고쳐져 `v3_wc/` + `v3_humid_wc/` + `v4_humid_wc/`를 전부
읽음).

| 구분 | 조성 |
|---|---|
| 건조·습윤 **둘 다** 있음 | `base`, `mslm075`, `saIm025`, `saIm050`, `saIm0583`, `saIm075`, `saIm100` (7개) |
| **습윤만** 있음(건조 WC 없음) | `mslm025`, `mslm050`, `nbIm075`, `nbIm100`, `saIm0625`, `ms50nb50`, `sa25nb75`, `sa50nb50` (8개) |
| **건조만** 있음(습윤 없음) | 없음 |

**이상 없음** — `regen_energy_v3.py`의 `NAMES`는 `dry_tsa/dry_vsa/wet_tsa
/wet_vsa`와 `Qst_CO2`가 **모두** 있는 조성만 골라 위 7개로 한정한다
(실제 `NAMES` 변수 값으로 확인). 습윤만 있는 8개 조성은 재생에너지 계산에
아예 들어가지 않으므로 반쪽 채워진 행이 완성된 행으로 보이는 상황은
지금 없다. (`v3_humid_wc_ens/`의 앙상블 멤버 `saIm050e1~5`·
`saIm0583e1~5`는 `load_wc()`가 읽는 경로에 포함되지 않아 별도 집계.)

## 사람이 볼 것

- **`laptop-20260822` 브랜치가 master 대비 450커밋(약 35.8시간) 뒤처짐** — 문턱(24) 대폭 초과. 핵심 교리 파일(CLAUDE.md·ERROR_BARS.md)은 무사하지만 최근 이틀 판정 맥락이 빠져 있음.
- **`audit.md` 자체가 2026-09-25 postman 자동 반입 커밋(`17fe6387`)에 175줄(09-25 감사 항목 전체)을 지워짐** — 결과 파일 반입 스크립트가 무관한 COMMS 파일을 건드린 사고, 복구 안 함(기록만).
- `v3_water_grid/`·`v3_water_grid_cliff/`에 태그 없는/있는 동일 내용 파일 쌍이 여전히 공존(08-23형 중복) — `machine_of()`가 지금은 막고 있으나 파일 정리는 안 됨.
