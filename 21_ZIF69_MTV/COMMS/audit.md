...
## 2026-09-25 00:14 UTC — 일일 감사

**실행 조건**: 클라우드 저장소 체크아웃만으로 확인. 로컬 프로세스·메모리·
미푸시 커밋은 볼 수 없음. 새 계산 없음. 아래 6개 점검 외
`21_ZIF69_MTV/COMMS/audit.md` 만 수정.

**방법 메모(경고 아님, 09-19~09-24 감사가 반복 잡은 것과 같은 함정 재확인)**:
이번 체크아웃도 얕은 클론으로 시작했다(`git rev-parse
--is-shallow-repository` → `true`). 얕은 상태에서 `git rev-list
--max-parents=0`을 세 브랜치에 각각 돌리니 **서로 다른 루트 커밋**이 나와
"master 와 laptop-20260822 가 공통 조상이 전혀 없는 별개 이력"으로 보였다
— `git fetch --unshallow`(2초) 후 재확인하니 **셋 다 같은 루트**(`6142e4f`)
였다. 얕은 클론이 브랜치마다 다른 깊이에서 잘려 있어 생긴 착시였다. **이
감사는 얕은 클론 상태에서 나온 브랜치 비교 수치를 신뢰하지 않는다** —
아래 1절 수치는 전부 `--unshallow` 이후 값이다.

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-25
00:14 UTC = 09:14 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `64e4150c` 2026-09-25T09:06:35+09:00 | 7분 | — | — |
| `origin/laptop-20260822` | `14a65d61` 2026-09-25T09:04:39+09:00 | 9분 | **135커밋** | 450커밋 |
| `origin/junseok-20260822` | `3648793e` 2026-09-25T09:06:50+09:00 | 7분 | 10커밋 | 118커밋 |

(`git rev-list --left-right --count origin/master...origin/<브랜치>`,
전체 이력 기준. master 전체 2,844커밋 · laptop-20260822 전체 3,159커밋 ·
junseok-20260822 전체 2,952커밋.)

**경고: `laptop-20260822` 가 master 대비 135커밋 뒤처짐 — 문턱(24) 초과.**
`merge-base` 는 `0be33d5c`(2026-09-24T21:22:59+09:00, "§-보완2 검산")이고,
그 이후 laptop 은 자기 커밋 450개(주로 `[postman:laptop] 결과 파일 자동
반입`)를 쌓았지만 master 의 새 커밋 135개를 다시 받아오지 않았다 — 마지막
동기화로부터 **약 11.85시간(710분)** 경과. **직접 확인**: 이 135개 중
50개가 `COMMS.md` 또는 `21_ZIF69_MTV/MAGI/` 아래 파일을 건드린다 — 오늘
저녁 `MAGI-005`(선택도 설계) 등록·해시 접수·시한 연장(01:00→02:00) 관련
커밋들이 laptop 쪽에 아직 없다는 뜻이다. **다만 08-24 사고와 달리 핵심
교리 파일은 영향이 없다**: `git log origin/laptop-20260822..origin/master
-- CLAUDE.md 21_ZIF69_MTV/ERROR_BARS.md` 가 0건이고, `git diff
origin/master:CLAUDE.md origin/laptop-20260822:CLAUDE.md` 와 같은 `diff
...:ERROR_BARS.md` 둘 다 **출력 없음(바이트 단위 동일, 33,976 B / 16,967
B)** — laptop 이 CLAUDE.md·ERROR_BARS.md 를 "없다"고 오인할 위험은 지금은
없다. 뒤처진 것은 MAGI-005 코디네이션 내용이다.

**이상 없음: `junseok-20260822`** 는 master 대비 10커밋 뒤처짐(문턱 24
미만) · `merge-base` 가 2026-09-25T08:07:20+09:00(약 1.11시간 전)로 정상
범위.

### 2) 우편함 침묵

각 기기는 자기 브랜치에 쓴다(`COMMS.md` §2). `desktop.md` 만 master 에
직접 쓴다. 각 파일을 그 기기 자신의 브랜치에서 확인(기준 09:14 KST):

| 파일 | 확인한 곳 | 마지막 커밋 | 경과 |
|---|---|---|---|
| `COMMS/desktop.md` | `origin/master` | 2026-09-25T08:24:20+09:00 | 49분 |
| `COMMS/laptop.md` | `origin/laptop-20260822` | 2026-09-25T09:03:52+09:00 | 10분 |
| `COMMS/junseok.md` | `origin/junseok-20260822` | 2026-09-25T08:21:14+09:00 | 52분 |
| `COMMS/laptop2.md` | `origin/laptop2-20260825` | 2026-09-25T08:25:34+09:00 | 48분 |
| `COMMS/cloud4c.md` | 전 브랜치 중 최신(`origin/master`) | 2026-09-25T04:36:48+09:00 | 4.62시간 |

이상 없음 — `desktop.md`·`laptop.md`·`junseok.md`·`laptop2.md` 넷 다
1시간 이내로 활발함. `cloud4c.md` 만 4.6시간 침묵이나, `cloud4c` 는
COMMS.md 규약상 "임시 기기"(수명이 짧고 데스크탑이 감시)라 이 경과만으로
죽었다고 단정하지 않음 — 침묵 사실과 경과 시간만 보고.

### 3) 결과 JSON 무결성

`results_v3*.json`(11개) + `v3_wc/`(2개) + `v3_humid_wc/`(7개) +
`v4_humid_wc/`(2개) + `v3_water_grid*/`(6개) = 총 28개 파일을 파이썬
`json.load` 로 파싱하고 행수(`rows` 키가 있으면 그 리스트 길이, 없으면
최상위 리스트 길이)를 셌다.

**이상 없음** — 28개 전부 파싱 성공, 0행 없음(최소 1행 `results_v3_smoke.json`,
최대 31행 `results_v3.json`).

### 4) 출처 규약 위반

`merge_water_batches.py::machine_of()` 는 `water_results_<기기>.json`
형식만 받고 태그 없는 파일은 `SystemExit` 로 거부한다(코드 자체가 08-23
이중 계수 사고 이후 이렇게 고쳐져 있음, 78~87행). `v3_water_grid*/` 안의
파일을 쌍으로 비교:

| 쌍 | 겹치는 (조성,RH) | 값 | 판정 |
|---|---|---|---|
| `v3_water_grid/water_results.json` ↔ `water_results_desktop4.json` | saIm0583 × 4개 RH | **완전 동일** | **중복**(같은 파일의 두 사본) |
| `v3_water_grid_cliff/water_results.json` ↔ `water_results_desktopcliff.json` | saIm0917·saIm0958 × 4개 RH | **완전 동일** | **중복**(같은 파일의 두 사본) |
| `v3_water_grid/water_results_junseok.json` ↔ `water_results_laptop.json` | saIm0875 × 4개 RH | **값이 다름**(예: RH0.9 CO2 0.9619 vs 0.9671, retention_sigma 8.67 vs 6.37) | **진짜 교차검증**(두 기기 독립 계산) |

이상 없음 — `machine_of()` 는 태그 없는 `water_results.json` 을 거부하므로
현재 코드 경로로는 이중 계수가 재현되지 않는다(직접 확인: 함수가 태그
없는 파일명에 `SystemExit` 를 던지는 분기를 코드에서 읽음). 다만 태그
없는 완전 중복 사본이 디스크에 남아 있어, **글롭으로 태그 없는 파일까지
쓸어 담는 다른 스크립트**가 있으면 여전히 위험하다 — 실제로
`check_denominators.py:35` 가 `*water*/water_results*.json` 로 그 함정을
**의도적으로 재현**해 보여 주는 진단 스크립트로 존재한다(자기 인식된
경고용, `os.chdir("/home/leehk/...")` 하드코딩이라 이 컨테이너에서 직접
돌지는 않음). 새 병합 스크립트를 짤 때는 `machine_of()` 를 재사용하거나
같은 거부 규칙을 넣을 것.

### 5) 사전 등록 관문 대비 기록

관문: RH90/RH0 ≥80% 유효 / 50~80% 조건부 / <50% 종료. `v3_water_grid*/`
의 **기기 태그 파일만**(중복 사본 제외) 써서 유지율을 직접 재계산:

| 조성 | 출처 | RH0 CO2(mol/kg) | RH90 CO2 | 유지율 | 관문 |
|---|---|---|---|---|---|
| saIm0583 | desktop4 | 1.3188 | 1.0010 | 75.90% | 조건부 |
| saIm0625 | laptop | 1.3804 | 0.9902 | 71.74% | 조건부 |
| saIm0667 | laptop | 1.3682 | 1.0269 | 75.06% | 조건부 |
| saIm0875 | junseok | 1.2905 | 0.9619 | 74.54% | 조건부 |
| saIm0875 | laptop | 1.3031 | 0.9671 | 74.21% | 조건부 |
| saIm0917 | desktopcliff | 1.3249 | 0.9725 | 73.41% | 조건부 |
| saIm0958 | desktopcliff | 1.5205 | 1.0032 | 65.98% | 조건부 |

계산값은 각 행의 `CO2_retention_pct` 필드와 일치(자체 계산 중복 아님,
이미 있는 자를 부름).

이상 없음 — 7행(조성 6종 + saIm0875 두 기기) 전부 저장소 문서(`COMMS/
desktop.md`·`laptop.md`·`junseok.md`)에 적힌 "조건부" 판정과 어긋나지
않음. 다만 **모든 RH>0 행에 `WARN_forcefield`(2026-09-05 발견, 수소결합
없는 물 결함판) 태그가 붙어 있음을 직접 확인** — 이 파일들은 09-06
수정 이전의 결함판 물 힘장 결과다. 저장소 문서는 이를 이미 알고 있고
(`v3w_*` 계열로 재계산 시 saIm0583 등이 조건부→유효로 재등급될 수 있다는
논의가 과거 감사 항목에 이미 기록돼 있음), 이번 감사 범위(`v3_water_grid*/`
만)에서는 문서화된 값과 재계산값이 일치하므로 새 경고로 올리지 않음 —
이미 추적 중인 사안.

### 6) WC 자료 공백

`regen_energy_v3.py::load_wc()` 가 실제로 읽는 파일 기준으로 `v3_wc/`
(건조)·`v3_humid_wc/`+`v4_humid_wc/`(습윤) 각각의 조성명을 추출:

| 조성 | 건조(v3_wc) | 습윤(v3_humid_wc/v4_humid_wc) |
|---|---|---|
| base | ✅ | ✅ |
| mslm075 | ✅ | ✅ |
| saIm025 | ✅ | ✅ |
...
건조+습윤 둘 다: **7종**(base, mslm075, saIm025, saIm050, saIm0583,
saIm075, saIm100). 습윤만 있는 것: **8종**(ms50nb50, mslm025, mslm050,
nbIm075, nbIm100, sa25nb75, sa50nb50, saIm0625). 건조만 있는 조성은 없음.

이상 없음 — `regen_energy_v3.py` 67~69행이 "`dry_tsa`·`dry_vsa`·`wet_tsa`·
`wet_vsa`·Q_st 다섯이 전부 있는 조성만" 필터링하도록(08-27 감사가 잡은
saIm0583 누락 사고 이후) 이미 고쳐져 있고, **직접 확인**: 현재
`regen_energy_v3.json` 의 실제 출력 행이 정확히 위 "건조+습윤 둘 다" 7종과
일치한다 — 습윤만 있는 8종은 절반만 찬 행으로 새지 않는다.

### 사람이 볼 것

- ⚠️ `laptop-20260822` 가 master 대비 **135커밋(11.85시간)** 뒤처짐 —
  그중 50개가 `MAGI-005`(오늘 저녁 시한 연장 포함) 관련. CLAUDE.md·
  ERROR_BARS.md 는 바이트 동일이라 무관하나, MAGI-005 코디네이션은 못 봄.
- `v3_water_grid*/` 안에 태그 없는 완전 중복 사본 2쌍(saIm0583,
  saIm0917/0958) 존재 — `machine_of()` 는 막아 주지만 새 글롭 스크립트는
  주의 필요.
- 나머지(우편함 침묵·JSON 무결성·관문 재계산·WC 공백)는 이상 없음.

---

## 2026-09-24 00:14 UTC — 일일 감사

**실행 조건**: 클라우드 저장소 체크아웃만으로 확인. 로컬 프로세스·메모리·
미푸시 커밋은 볼 수 없음. 새 계산 없음. 아래 6개 점검 외
`21_ZIF69_MTV/COMMS/audit.md` 만 수정.

**방법 메모(경고 아님)**: 이번 체크아웃도 얕은 클론으로 시작했다
(`git rev-parse --is-shallow-repository` → `true`). `git fetch --unshallow`
로 전체 이력(2,165+커밋, 2026-08-03 `Initial checkpoint`까지)을 받은 뒤
아래 수치를 냈다 — 09-19~09-23 감사가 반복 잡은 것과 같은 함정이라
다시 확인만 해 둔다.

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-24
00:14 UTC = 09:14 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `92eb21f` 2026-09-24T08:36:17+09:00 | 0.63시간 | — | — |
| `origin/laptop-20260822` | `df2b921` 2026-09-24T09:01:23+09:00 | 0.22시간 | **441커밋** | 377커밋 |
| `origin/junseok-20260822` | `7746631` 2026-09-24T08:29:18+09:00 | 0.75시간 | **6커밋** | 10커밋 |

**경고(신규, 09-23 감사의 "1커밋 뒤처짐·이상없음"에서 급변): `laptop-20260822`가
master 대비 **441커밋** 뒤처짐 — 문턱(24) 대폭 초과.** `merge-base`는
`cc4547c`(2026-09-23 10:43:54 KST, "postman 결함 번호 정정")이고, 그 이후
master 는 441커밋을 더 쌓았는데 `laptop-20260822`는 자기 커밋 377개만
쌓고 master 를 다시 받아오지 않았다. `laptop-20260822`의 커밋 로그를
직접 보면 마지막 `Merge remote-tracking branch 'origin/master'`가
**2026-09-23 10:44:48 KST**(지금부터 약 **22.5시간 전**)이고, 그 뒤로는
쭉 자기 `[postman:laptop] 결과 파일 자동 반입` 커밋만 반복되고 있다 —
laptop 이 master 를 22.5시간째 못(또는 안) 받아가고 있다는 뜻이고, 이는
08-24 사고(랩탑이 36커밋 뒤처져 `ERROR_BARS.md`를 "없다"고 오인)와 같은
유형이다. **직접 확인**: `git diff origin/laptop-20260822 origin/master
-- CLAUDE.md`를 뜨면 64줄 차이가 나고, laptop 쪽 `CLAUDE.md`에는 오늘
(2026-09-24) 추가된 경고 5건 — "우연이 지켜 주고 있는 자리", "지표는
0과 아직 0", "조건문과 관문은 모든 길을", "대역은 의도대로 안 되는 게
기본", "자기가 쓴 규율을 자기 새 코드에" — 이 **전부 빠져 있다.** laptop
에서 도는 세션이 있다면 지금 이 다섯 경고를 못 보고 있다.

**주의(신규, 지난 감사들의 "junseok 은퇴" 를 정정): `junseok-20260822`는
master 대비 6커밋 뒤처짐·10커밋 앞섬으로 정상 범위이고, **09-19~09-23
감사가 반복해 "은퇴"라고 적었던 상태는 저장소 자체가 오늘 정정했다.**
`COMMS.md` 449행과 `21_ZIF69_MTV/ASSIGN_JUNSEOK_20260924.md`(데스크탑,
오늘 07:35)에 "**'은퇴'는 감사의 추론이었습니다 — 결정한 사람이 없었고
등록부는 🟢 그대로였습니다**"라고 명시돼 있다. 즉 **이전 감사들이 25일
침묵과 merge-base 위치만 보고 "은퇴"로 단정한 것 자체가 틀렸고**, 그
틀린 판정을 6회 연속 반복 게재했다 — 이 감사 로그 자신이 §0 이 경고하는
"설명이 숫자를 맞히면 의심하라" 유형의 사례가 됐다. junseok 은 실제로는
09-24 07:19 재부팅해 힘장 관문을 자가 복구(md5 73046fd7→8e8ec933)했고
07:35 새 배정(밀도맵 mslm 사다리 완성 + cf3Im 신설)을 받았다. **이번
감사부터 "은퇴"라는 말을 쓰지 않는다** — 등록부 상태(🟢)를 그대로 보고한다.

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825`도 오늘 활발히
움직이고 있음을 2절에서 확인했다. `origin/junseok`(접미사 없음)·
`origin/magi004-*` 3종·`origin/claude/essential-programs-install-ur7v6e`는
지시된 점검 대상이 아니어서 상세 비교는 생략.

### 2) 우편함 침묵

⚠️ **master 만 보면 안 된다** — `COMMS.md` §2 규약대로 각 기기는 자기
우편함을 **자기 브랜치에만** 쓰고, postman 의 결과 반입은 `RESULT_PATTERNS`
(JSON 류)만 옮기지 `COMMS/*.md` 는 옮기지 않는다(직접 확인: 최신 반입
커밋 `e41758f`의 diff 는 `MACHINE_CAPABILITIES.md` 만 건드리고 `junseok.md`
는 안 건드림). 그래서 **master 기준 타임스탬프는 기기 우편함에 대해
구조적으로 낡아 보인다** — 이것은 설계이지 침묵이 아니다. 각 파일을
그 기기 자신의 브랜치에서 봤다:

| 파일 | 확인한 곳 | 마지막 커밋 | 경과 |
|---|---|---|---|
| `COMMS/desktop.md` | `master` (desktop 는 직접 master 에 씀) | 2026-09-24T08:00:32+09:00 | 1.23시간 |
| `COMMS/laptop.md` | `origin/laptop-20260822` | 2026-09-24T07:36:04+09:00 | 1.62시간 |
| `COMMS/junseok.md` | `origin/junseok-20260822` | 2026-09-24T08:29:18+09:00 | 0.75시간 |
| `COMMS/laptop2.md` | `origin/laptop2-20260825` | 2026-09-24T08:13:19+09:00 | 1.02시간 |
| `COMMS/cloud4c.md` | `master`(별도 브랜치 없음) | 2026-09-04T17:49:13+09:00 | 470.4시간(19.6일) |

**이상 없음(활동 기기)** — desktop·laptop·junseok·laptop2 넷 다 자기
브랜치 기준 1.62시간 이내로 정상 활동. **참고(반복, 새 이상 아님)**:
`cloud4c.md`는 19.6일째 무갱신 — `COMMS.md` 등록부에 별도 은퇴 기록이
있는 채널이라 "죽었다"가 아니라 등록된 정지 상태로 본다(계산 중 침묵과
구분해 단정하지 않음).

**주의**: master 만 봤다면 laptop.md 는 09-21 15:12(과거 감사가 보고한
값), junseok.md 는 08-29 11:32, laptop2.md 는 09-10 01:13 로 나와 셋 다
"수일~수주 침묵"으로 잘못 읽힌다 — 1절의 laptop 지연과 별개로, **우편함
자체는 postman 설계상 절대 master 에 반입되지 않으므로 master 기준 조회는
이 항목에 원래 안 맞는 방법**이다. 오늘부터 기기별 브랜치를 직접 봐서
교정한다.
...
총 27개 JSON 을 전부 파이썬 `json.load` 로 파싱하고 (dict 형태는) `rows`
...
09-23 감사와 파일 구성·행수가 완전히 동일(그 사이 새 결과 JSON 없음).

### 4) 출처 규약 위반

`merge_water_batches.py`의 `machine_of()`(태그 없는 `water_results.json`을
`SystemExit`으로 거부하는 코드를 직접 확인, 실행은 안 함)를 기준으로,
태그 파일 4개(`v3_water_grid/water_results_desktop4.json`,
...
- `v3_water_grid/water_results.json`(미태그, 4행, `saIm0583` RH
  0/0.25/0.5/0.9) == `water_results_desktop4.json` — 4행 전부
  `CO2_molkg`·`H2O_molkg`까지 완전히 동일.
- `v3_water_grid_cliff/water_results.json`(미태그, 4행, `saIm0917`·
  `saIm0958` RH 0/0.9) == `water_results_desktopcliff.json` — 4행 전부
  동일.

`machine_of()`가 미태그 파일을 `SystemExit`으로 거부하므로, 병합
스크립트에 **태그 파일만** 넘기는 한 이중 계수로 이어지지 않는다 —
지금 저장소 상태에서 활성 이중 계수는 확인되지 않았다. 다만 미태그
사본이 남아 있는 구조적 위험은 그대로다(09-19 감사부터 동일).

**진짜 교차검증(중복 아님)**: `saIm0875`가 `water_results_junseok.json`과
`water_results_laptop.json` 양쪽에 **다른 값**으로 존재한다(RH0/RH90
CO2 가 4점 전부 소수점 단위까지 다름) — 두 기기가 같은 조성을 독립으로
쟀다. 값이 완전히 같은 위 두 쌍과는 달리 이것은 duplication 이 아니라
실제 독립 재현이다.

### 5) 사전 등록 관문 대비 기록

`v3_water_grid*/`의 태그 파일(및 §4에서 값이 동일함을 확인한 미태그
사본)에서 조성별 RH0→RH0.9 의 `CO2_molkg` 비를 직접 재계산해 관문(≥80%
유효 / 50~80% 조건부 / <50% 종료)을 다시 적용하고, 각 행에 저장된
`CO2_retention_pct` 필드와도 대조했다.

| 조성 | 출처 | RH0 | RH0.9 | 유지율(재계산) | 저장된 값과 일치 | 관문 |
|---|---|---|---|---|---|---|
| `saIm0583` | desktop4/미태그 | 1.3188 | 1.0010 | 75.90% | ✅ | 조건부 |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.74% | ✅ | 조건부 |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.06% | ✅ | 조건부 |
| `saIm0875` | junseok | 1.2905 | 0.9619 | 74.54% | ✅ | 조건부 |
| `saIm0875` | laptop | 1.3031 | 0.9671 | 74.21% | ✅ | 조건부 |
| `saIm0917` | desktopcliff/미태그 | 1.3249 | 0.9725 | 73.41% | ✅ | 조건부 |
| `saIm0958` | desktopcliff/미태그 | 1.5205 | 1.0032 | 65.98% | ✅ | 조건부 |

**이상 없음** — `v3_water_grid*/`의 7개 (조성·기기) 조합 전부 저장된
`CO2_retention_pct`와 소수점까지 일치하고, 전부 50~80% 구간(조건부)이라
`COMMS/desktop.md`·`laptop.md`·`junseok.md`에 적힌 "조건부" 판정과도
어긋나지 않는다(grep 으로 직접 대조: "75.90 ± 1.99%, 조건부" 등). 09-19
감사 이후 값·판정 모두 변화 없음.

참고(이 점검 범위 밖): 저장소에는 09-06 물 힘장 수정 이후 계열로 보이는
`v3w_water/`도 별도로 존재하고, 과거 감사가 그쪽에서 일부 조성(`saIm0583`
87%대 등)이 유효(≥80%) 쪽으로 올라간다고 적어 둔 바 있다. 오늘 지시받은
점검 범위는 `v3_water_grid*/` 로 한정돼 있어 `v3w_water/`는 다시 열지
않았다 — 두 계열을 섞어 인용하지 말 것(CLAUDE.md 2절, 08-14 유형).

### 6) WC 자료 공백

`regen_energy_v3.py`를 모듈로 임포트해(`main()` 미실행) 실제 `NAMES`를
직접 조회했다. `v3_wc/`·`v3_humid_wc/`·`v4_humid_wc/`의 `name` 목록도
JSON 에서 직접 집계했다.
...
- **습윤만 있음(8)**: `mslm025`, `mslm050`, `ms50nb50`, `nbIm075`,
  ...
  `NAMES`에서 자동 제외됨(임포트로 확인, 절반만 찬 행이 만들어지지 않음).
- **건조만 있음(0)**: 없음.

**이상 없음** — 필터가 코드로 걸려 있어 지금 당장 "절반만 찬 행"이
만들어지지는 않는다. 습윤만 있는 8개 조성(특히 route-B 혼합 `ms50nb50`·
`sa25nb75`·`sa50nb50`)은 건조 WC 가 등록되기 전까지 `regen_energy_v3.json`에
아예 나타나지 않으므로, 그 출력만 보면 "재생에너지가 나쁘다"가 아니라
"아직 안 실렸다"로 읽어야 한다. 09-19 감사 이후 변화 없음.

### 사람이 볼 것

- **경고(신규): `laptop-20260822`가 master 를 22.5시간째 못 받아가
  441커밋 뒤처짐 — 오늘(09-24) 추가된 CLAUDE.md 경고 5건을 그 기기
  세션이 못 보고 있음. postman/수동 pull 확인 필요.**
- **정정(신규): 09-19~09-23 감사가 반복 게재한 "junseok 은퇴"는 이
  감사 자신의 잘못된 추론이었음 — 저장소가 오늘 정정(등록부 줄곧 🟢,
  09-24 07:19 복귀·재배정). 앞으로 "은퇴" 표현을 쓰지 않음.**
- 참고(경고 아님, 지속): `v3_water_grid*/`의 미태그 `water_results.json`
  2개가 태그 파일과 완전 동일 — `machine_of()`가 막아 지금은 안전.
...
**방법 메모(경고 아님, 09-19~09-21 감사가 잡은 것과 같은 검사기 함정
재확인)**: 이번 체크아웃도 얕은 클론으로 시작했다(`git rev-parse
--is-shallow-repository` → `true`). 언셸로우 전 `git fetch --all`이
`origin/master`를 "forced update"로 보고했고(로컬 얕은 참조 `e86f98d` →
`803536a`), `git merge-base origin/master origin/laptop-20260822`와
`origin/master origin/junseok-20260822` 둘 다 **공통 조상 없음(exit 1)**을
냈다 — `master`의 얕은 역사가 오늘 `postman` 반입 커밋 50개(전부
`[postman:desktop] ... 결과 반입`, 가장 오래된 것이 `0e3dc06`
2026-09-22 05:50:24 KST, **부모 커밋 없음**으로 보임)로만 잘려 있어,
잠깐 "`master`의 전체 역사가 사라졌다"는 실패 신호처럼 보였다. `git
fetch --unshallow` 후 재확인하니 `master`는 실제로 2,012커밋
(2026-08-03 `Initial checkpoint`부터)이 전부 살아 있고 `0e3dc06`도
정상 부모(`daf7932`)를 가진다 — **저장소 손상이 아니라 얕은 클론에서
`merge-base`/`rev-list`가 내는 값을 못 믿는다는, 이 감사가 반복 확인해 온
바로 그 함정이었다.** 아래 1·2절 수치는 전부 `--unshallow` 이후 값이다.

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-22
00:09 UTC = 09-22 09:09 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `803536a` 2026-09-22 00:05:21 UTC (09:05:21 KST) | 0.06시간 | — | — |
| `origin/laptop-20260822` | `41122e8` 2026-09-22 00:04:52 UTC (09:04:52 KST) | 0.07시간 | **286커밋** | 147커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (08-29 00:54:23 KST) | **584.24시간(24.34일)** | **1,603커밋** | 0커밋 |

**경고(신규·확대): `laptop-20260822`가 master 대비 286커밋 뒤처짐 —
08-24 사고 기준(24커밋)의 12배, 09-21 감사(0커밋, "master 최신 끝점과
병합 확인")에서 하루도 안 돼 급격히 벌어졌다.** `git merge-base`
(`7aad1b6`, 2026-09-21 06:18:49 UTC 커밋 — "[랩탑] postman 자기갱신에
`bash -n` 관문")를 기준으로 보면 두 브랜치가 갈라진 지 **17.9시간**
지났다. 이 사이 `laptop-20260822` 자신은 죽지 않았다 — 마지막 커밋이
4분 전이고 자기 커밋을 147개 더 쌓았다(활발히 도는 중). 반대로
`master` 쪽은 같은 창에서 postman 반입으로 286커밋 전진했는데
`laptop-20260822`가 그동안 `master`를 한 번도 받아오지 않았다.
`POSTMAN.md`/`postman.sh`의 `runner_running()` 가드(러너가 도는 동안
pull/merge를 건너뜀)가 설계대로 작동한 결과일 가능성이 높다 — 즉
"기기가 죽었다"가 아니라 "장시간 계산 중이라 동기화를 계속 미루고
있다"는 그림과 일치한다. 다만 **이 감사는 로컬 프로세스를 볼 수 없어
어느 쪽인지 확정할 수 없고, 지시된 문턱(24커밋)을 넘었으므로 경고로
올린다.** 08-24 사고가 바로 "뒤처짐을 몰라서 옛 문서를 없다고 오인"한
것이므로, `laptop-20260822`를 인용할 사람은 `master`의 최신본이 아니라
**뒤처진 그 커밋 시점 기준**임을 감안해야 한다.

**경고(반복, 09-19 감사부터 지속 — 새 이상 아님): `junseok-20260822`가
마지막 커밋 24.34일 경과, master 대비 1,603커밋 뒤처짐.** `merge-base`가
`junseok-20260822` 자기 끝점(`41fb1b7`)과 정확히 같고 앞섬이 0커밋이라,
이 브랜치 내용은 이미 전부 `master`에 흡수돼 있다 — 08-29 자진 종결
확인(`COMMS/junseok.md` "가지 취합 종결 확인") 이후 새 활동이 없는
상태가 계속 이어지고 있다. 08-24 유형의 "놓친 뒤처짐"이 아니라 은퇴된
브랜치이지만, 지시된 문턱(24커밋)을 그대로 적용해 경고로 남긴다.
...
`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋,
`--unshallow` 이후) 기준, 기준 시각 2026-09-22 00:09 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 경과 |
|---|---|---|
| `COMMS/desktop.md` | `c54daf0` 2026-09-21 05:29:22 UTC | 18.66시간 |
| `COMMS/laptop.md` | `3e8f4f2` 2026-09-21 14:12:34 UTC | 9.94시간 |
| `COMMS/laptop2.md` | `a208d15` 2026-09-21 14:20:12 UTC | 9.82시간 |
| `COMMS/junseok.md` | `be2bede` 2026-08-29 02:32:05 UTC | 573.62시간(23.90일) |
| `COMMS/cloud4c.md` | `84cfdde` 2026-09-04 08:49:13 UTC | 423.33시간(17.63일) |
| `COMMS/external16.md` | (모든 브랜치를 통틀어도 커밋된 적 없음) | — |
| `COMMS/audit.md`(이 파일, 참고용) | `d3c8024` 2026-09-21 00:11:04 UTC | 23.97시간 |

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. **이상
없음** — `desktop.md`·`laptop.md`·`laptop2.md`는 18.66시간 이내로 정상
활동. `junseok.md`(23.90일)·`cloud4c.md`(17.63일)는 각각 1절의
`junseok-20260822` 은퇴, `COMMS.md` 등록부 "⚪ 완료" 상태와 같은 방향의
지속 침묵으로 09-19 감사부터 반복 확인된 상태(새 이상 아님).
`external16.md`는 등록부에도 "⚪ 완료"로 은퇴돼 있어 "무응답"이 아니라
"우편함이 애초에 없음".
...
**이상 없음** — 27개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
09-21 감사와 파일 구성·행수가 완전히 동일(그 사이 새 결과 JSON 없음).
...
같은 조성을 독립으로 쟀다.
    ...
    RH    junseok CO2_molkg   laptop CO2_molkg   junseok H2O    laptop H2O
    0.0   1.2904819813        1.3031497452       0.0            0.0
    0.25  1.1742841149        1.1497682605       0.8667428793   0.9239063874
    0.5   1.0563622055        1.0582424665       1.3371243096   1.3315315215
    0.9   0.9619049737        0.9670695743       1.7280868312   1.7093519792

값이 갈리므로(RH90 유지율 74.54% vs 74.21%, §5 참조) 08-23 유령
교차검증(0.00σ 완전 일치)과는 다른, 실제 독립 재현이다.

### 5) 사전 등록 관문 대비 기록

`v3_water_grid*/`의 태그 파일(및 태그 파일과 값이 동일함이 §4에서
확인된 미태그 사본)에서 조성별 RH0→RH0.9의 `CO2_molkg` 비(파일 자체의
`CO2_retention_pct` 필드와 대조 후 재계산)로 관문(≥80% 유효 / 50~80%
조건부 / <50% 종료)을 다시 적용하고, `COMMS/desktop.md`·
`COMMS/laptop.md`에서 해당 수치를 `grep`으로 직접 대조했다.
...
**이상 없음** — `v3_water_grid*/`의 7개 (조성·기기) 조합 전부 50~80%
구간(조건부)이고, 값과 관문 판정 모두 저장소 문서와 어긋나지 않는다.
(참고: `saIm0958`를 "62.6~62.7%"로 적은 `COMMS/laptop2.md`는 이 파일이
아니라 laptop2의 별도 실현·9조각 창 평균을 인용한 것으로 확인함 — 09-19
감사부터 이미 서로 다른 측정치로 구분해 둔 상태.)

### 6) WC 자료 공백

`regen_energy_v3.py`를 모듈로 임포트해(`main()` 미실행) 실제 `NAMES`를
직접 조회했다 — `{'dry_tsa','dry_vsa','wet_tsa','wet_vsa'} <= set(v) and
n in QST`(67~69행) 필터가 그대로 걸려 있음을 코드로도 확인했고, 실제
반환값이 아래 "둘 다 있음" 7개와 정확히 일치함을 확인했다. `v3_wc/`·
`v3_humid_wc/`·`v4_humid_wc/`의 `name` 목록도 별도로 직접 집계했다.
...
`regen_energy_v3.json`에 아예 나타나지 않는다 — 사람이 그 출력만 보면
"이 조성들은 재생에너지가 나쁘다"가 아니라 "아직 안 실렸다"로 읽어야
한다. 09-19~09-21 감사와 동일한 결론, 변화 없음.

### 사람이 볼 것

- **경고(신규·확대)**: `laptop-20260822`가 09-21 감사의 "0커밋 뒤처짐"에서
  하루 안 돼 **286커밋 뒤처짐**(문턱 24 초과)으로 급격히 벌어짐. 갈라진
  지 17.9시간, 자기 브랜치는 4분 전까지 활동 중 — 죽은 것이 아니라
  `postman`의 `runner_running()` 가드로 동기화가 계속 미뤄지고 있을
  가능성이 높으나 로컬 프로세스를 볼 수 없어 확정 불가.
- 경고(반복, 새 이상 아님): `junseok-20260822`가 24.34일째 무갱신·
  master 대비 1,603커밋 뒤처짐이나 `merge-base`가 자기 끝점이라 이미
  전부 `master`에 흡수·은퇴된 상태(08-29 종결 확인, 09-19 감사부터
  연속 확인 중).
- 참고(경고 아님, 지속): `v3_water_grid*/`의 미태그 `water_results.json`
  2개가 태그 파일과 행 단위로 완전 동일 — `machine_of()`가 막아 지금은
  안전하나 글롭으로 같이 넘기면 이중 계수 재발하는 구조는 그대로.
...
**방법 메모(경고 아님, 09-19·09-20 감사가 잡은 것과 같은 검사기 함정
재확인)**: 이번 체크아웃도 얕은 클론으로 시작했다(`git rev-parse
--is-shallow-repository` → `true`). 언셸로우 전 `git merge-base
origin/master origin/junseok-20260822`는 **공통 조상 없음(오류)**을
냈고 `rev-list --left-right --count`는 `50 409`를 냈다 — 완전히 분리된
두 역사처럼 보였다(같은 명령을 `laptop-20260822`에 걸었을 때는 우연히
`merge-base`가 이미 `master` 끝점과 같게 나와 해가 덜 드러났다). `git
fetch --unshallow` 후 다시 재니 `junseok-20260822`의 `merge-base`가 그
브랜치 자기 끝점(`41fb1b7`)을 그대로 반환했다(= 이미 `master`에 전부
흡수돼 있음). **아래 1·2절 수치는 전부 `--unshallow` 이후 값이다.**

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-21
00:09 UTC = 09-21 09:09 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `b749519` 2026-09-20 22:01:18 UTC (09-21 07:01:18 KST) | 2.1시간 | — | — |
| `origin/laptop-20260822` | `5ba09fd` 2026-09-21 00:01:02 UTC (09:01:02 KST) | 0.1시간 | **0커밋** | 21커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (08-29 00:54:23 KST) | **560.2시간(23.34일)** | **1213커밋** | 0커밋 |

**이상 없음: `laptop-20260822`가 master 대비 0커밋 뒤처짐.** `merge-base`가
`master`의 현재 끝점(`b749519`) 그 자체 — 09-20 감사가 경고했던 25커밋
뒤처짐이 오늘 해소됐고, 그 위에 자기 커밋 21개를 더 쌓았다(마지막 커밋
6분 전, 활동 중).

**경고(반복, 09-19 감사부터 계속 확인 중인 상태 — 새 이상 아님):
`junseok-20260822`가 마지막 커밋 23.34일 경과, master 대비 1213커밋
뒤처짐** — 08-24 사고 기준(24커밋)의 50배. 그러나 `merge-base`가
`junseok-20260822` 자기 끝점(`41fb1b7`) 그 자체이고 앞섬이 0커밋이라,
이 브랜치 내용은 이미 전부 `master`에 흡수돼 있다 — 08-29 자진 종결
확인(`COMMS/junseok.md` "가지 취합 종결 확인") 이후 새 활동이 없는
상태가 어제·그제 감사와 동일하게 유지되고 있다. 08-24 유형의 "놓친
뒤처짐"이 아니라 은퇴된 브랜치이지만, 지시된 문턱(24커밋)을 그대로
적용해 경고로 남긴다.

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825` 최근 커밋 존재
(아래 2절), `origin/junseok`(접미사 없음)·`origin/magi004-*` 3종·
`origin/claude/essential-programs-install-ur7v6e`도 존재하나 지시된
점검 대상이 아니어서 상세 비교는 생략.
...
`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋,
`--unshallow` 이후) 기준, 기준 시각 2026-09-21 00:09 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 경과 |
|---|---|---|
| `COMMS/desktop.md` | `39c2e4a` 2026-09-20 07:35:37 UTC | 16.5시간 |
| `COMMS/laptop.md` | `a1fa0ee` 2026-09-20 07:34:36 UTC | 16.5시간 |
| `COMMS/laptop2.md` | `1950496` 2026-09-20 22:02:48 UTC | 2.0시간 |
| `COMMS/junseok.md` | `be2bede` 2026-08-29 02:32:05 UTC | 549.6시간(22.90일) |
| `COMMS/cloud4c.md` | `84cfdde` 2026-09-04 08:49:13 UTC | 399.3시간(16.63일) |
| `COMMS/external16.md` | (저장소에 커밋된 적 없음) | — |
| `COMMS/audit.md`(이 파일, 참고용) | `974af29` 2026-09-20 00:13:00 UTC | 23.9시간 |

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. **이상
없음** — `desktop.md`·`laptop.md`·`laptop2.md`는 16.5시간 이내로 정상
활동. `junseok.md`(22.90일)는 1절의 `junseok-20260822` 은퇴와 같은
방향으로, 08-29 자진 종결 확인 이후의 침묵이라 이미 여러 차례 확인된
상태(새 이상 아님). `cloud4c.md`(16.63일)는 `COMMS.md` 등록부에 "⚪
완료"로 이미 은퇴 처리된 임시 기기라 경고 대상이 아니다.
`external16.md`는 `git log --all`로도 이 경로를 건드린 커밋이 없다 —
등록부에도 "⚪ 완료"로 은퇴돼 있어 "무응답"이 아니라 "우편함이 애초에
없음".
...
**이상 없음** — 27개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
09-20 감사와 파일 구성·행수가 완전히 동일(그 사이 새 결과 JSON 없음).
...
- `v3_water_grid/water_results.json` (미태그, 4행, `saIm0583` RH
  0/0.25/0.5/0.9) == `v3_water_grid/water_results_desktop4.json` —
  4행 전부 `CO2_molkg`·`H2O_molkg`까지 바이트 단위로 동일.
- `v3_water_grid_cliff/water_results.json` (미태그, 4행, `saIm0917`·
  `saIm0958` RH 0/0.9) == `v3_water_grid_cliff/water_results_
  desktopcliff.json` — 4행 전부 동일.
...
|---|---|---|---|---|---|---|
| `saIm0583` | desktop4/미태그 | 1.3188 | 1.0010 | 75.90% | 조건부 | `COMMS/desktop.md`("75.90 ± 1.99%, 조건부") 일치 |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.74% | 조건부 | `COMMS/laptop.md`("71.7% → 조건부") 일치 |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.06% | 조건부 | `COMMS/desktop.md`("75.1%, 조건부") 일치(반올림差 0.04p) |
| `saIm0875` | junseok | 1.2905 | 0.9619 | 74.54% | 조건부 | `COMMS/junseok.md`·`desktop.md`("74.5%, 조건부") 일치 |
| `saIm0875` | laptop | 1.3031 | 0.9671 | 74.21% | 조건부 | `COMMS/laptop.md`("74.2 ± 3.9 pp") 일치(§4 교차검증 값) |
| `saIm0917` | desktopcliff/미태그 | 1.3249 | 0.9725 | 73.41% | 조건부 | `COMMS/desktop.md`("절벽 유지율 0917 73.4") 일치 |
| `saIm0958` | desktopcliff/미태그 | 1.5205 | 1.0032 | 65.98% | 조건부 | `COMMS/desktop.md`("ret90(saIm0958)=65.98") 일치 |

**이상 없음** — `v3_water_grid*/`의 7개 (조성·기기) 조합 전부 50~80%
구간(조건부)이고, 값과 관문 판정 모두 저장소 문서와 어긋나지 않는다.
(참고: `saIm0958`를 "62.6~62.7%"로 적은 `COMMS/laptop2.md`는 이 파일이
아니라 laptop2의 별도 실현·9조각 창 평균을 인용한 것으로,
`RH90_DENOMINATORS_20260906.md`가 이미 "`v3_water_grid_cliff/
water_results.json` 단일"이라고 출처를 구분해 뒀다 — 모순이 아니라
서로 다른 측정치, 09-19 감사부터 동일.)

### 6) WC 자료 공백

`regen_energy_v3.py`를 모듈로 임포트해(`main()` 미실행) 실제 `NAMES`를
직접 조회했다 — `{'dry_tsa','dry_vsa','wet_tsa','wet_vsa'} <= set(v) and
n in QST`(67~69행) 필터가 그대로 걸려 있음을 코드로도 확인. `v3_wc/`·
`v3_humid_wc/`·`v4_humid_wc/`의 `name` 목록도 별도로 직접 집계했다.
...
`regen_energy_v3.json`에 아예 나타나지 않는다 — 사람이 그 출력만 보면
"이 조성들은 재생에너지가 나쁘다"가 아니라 "아직 안 실렸다"로 읽어야
한다. 09-19·09-20 감사와 동일한 결론, 변화 없음.

### 사람이 볼 것

- 경고(반복, 새 이상 아님): `junseok-20260822`가 23.34일째 무갱신·
  (기계적용 문턱 기준) master 대비 1213커밋 뒤처짐이나, `merge-base`가
  자기 끝점이라 이미 전부 `master`에 흡수·은퇴된 상태(08-29 종결 확인,
  09-19 감사부터 연속 확인 중).
- 참고(경고 해소): `laptop-20260822`가 09-20 감사의 25커밋 뒤처짐 경고에서
  오늘 **0커밋 뒤처짐**으로 해소됨(master 최신 끝점 병합 확인, 활동 중).
- 참고(경고 아님, 지속): `v3_water_grid*/`의 미태그 `water_results.json`
  2개가 태그 파일과 행 단위로 완전 동일 — `machine_of()`가 막아 지금은
  안전하나 글롭으로 같이 넘기면 이중 계수 재발하는 구조는 그대로.
...
## 2026-09-20 00:10 UTC — 일일 감사

**실행 조건**: 클라우드 저장소 체크아웃만으로 확인. 로컬 프로세스·메모리·
미푸시 커밋은 볼 수 없음. 새 계산 없음 — `merge_water_batches.py`·
`regen_energy_v3.py`는 소스만 읽고 실행하지 않음(둘 다 출력 파일을 씀).
아래 6개 점검 외 어떤 파일도 수정하지 않음.

**방법 메모(경고 아님, 09-19 감사가 이미 잡은 검사기 함정과 같은 자리에서
재확인)**: 이번 체크아웃도 얕은 클론으로 시작했다(`git rev-parse
--is-shallow-repository` → `true`). 언셸로우 전 `git merge-base
origin/junseok-20260822 origin/master` 는 **공통 조상 없음(오류)** 을
냈고 `rev-list --left-right --count` 는 409/77 을 냈다 — 완전히 분리된
두 역사처럼 보였다. `git fetch --unshallow` 후 다시 재니 `merge-base` 가
`junseok-20260822` 자기 끝점(`41fb1b7`)을 그대로 반환했다(= 이 브랜치는
이미 `master` 에 전부 흡수돼 있음, 09-19 감사·`COMMS/junseok.md` 08-29
"가지 취합 종결 확인" 과 일치). 같은 이유로 `git log --all -- <path>` 도
얕은 상태에서 `junseok.md`·`cloud4c.md` 의 마지막 갱신 커밋을 실제보다
훨씬 최근인 얕은-역사 경계 커밋(`524dfdf`, 09-19 15:38)으로 잘못
가리켰다. **아래 1·2절 수치는 전부 `--unshallow` 이후 값이다.**

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-20
00:10 UTC = 09-20 09:10 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `6f9fd44` 2026-09-20 00:05:34 UTC (09:05:34 KST) | 0.1시간 | — | — |
| `origin/laptop-20260822` | `5c63395` 2026-09-20 00:01:00 UTC (09:01:00 KST) | 0.2시간 | **25커밋** | 12커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (08-29 00:54:23 KST) | **536.3시간(22.35일)** | **1095커밋** | 0커밋 |

**경고: `laptop-20260822`가 master 대비 25커밋 뒤처짐 — 08-24 사고 기준
(24커밋) 을 넘었다.** 다만 마지막 커밋이 9분 전으로 활동 중단은 아니다
(`merge-base` `8fa6d07d`, 09-19 감사의 35커밋보다 오히려 줄었다 — master
쪽 postman 반입 속도와 laptop 쪽 자기 커밋 속도가 같이 움직인 결과로
보이며, 벌어지는 추세는 아니다).

**경고(문턱 기계적용, 새 이상 아님): `junseok-20260822`가 마지막 커밋
22.35일 경과, master 대비 1095커밋 뒤처짐.** 그러나 `merge-base` 가
`junseok-20260822` 자기 끝점(`41fb1b7`) 그 자체이고 앞섬이 0커밋이라,
이 브랜치 내용은 이미 전부 `master` 에 흡수돼 있다 — 09-09 감사부터
반복 확인된 08-29 자진 종결 상태 그대로다(`COMMS/junseok.md` "가지 취합
종결 확인"). 08-24 유형의 "놓친 뒤처짐" 이 아니라 은퇴된 브랜치이며,
지시된 문턱을 그대로 적용해 경고로만 남긴다.

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825` 최근 커밋 존재,
`origin/junseok`(접미사 없음)·`origin/magi004-*` 3종·
`origin/claude/essential-programs-install-ur7v6e` 도 존재하나 지시된
점검 대상이 아니어서 상세 비교는 생략.

### 2) 우편함 침묵

`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋, `--unshallow`
이후) 기준, 기준 시각 2026-09-20 00:10 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 경과 |
|---|---|---|
| `COMMS/desktop.md` | `a832e47` 2026-09-19 21:08:13 UTC | 3.0시간 |
| `COMMS/laptop.md` | `3c48770` 2026-09-19 15:13:16 UTC | 9.0시간 |
| `COMMS/laptop2.md` | `8b99b52` 2026-09-19 15:32:09 UTC | 8.6시간 |
| `COMMS/junseok.md` | `be2bede` 2026-08-29 02:32:05 UTC | 525.6시간(21.9일) |
| `COMMS/cloud4c.md` | `84cfdde` 2026-09-04 08:49:13 UTC | 375.4시간(15.6일) |
| `COMMS/external16.md` | (저장소에 커밋된 적 없음) | — |

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. **이상
없음** — `desktop.md`·`laptop.md`·`laptop2.md`는 9시간 이내로 정상.
`junseok.md`(21.9일)는 1절의 `junseok-20260822` 은퇴와 같은 방향으로,
08-29 자진 종결 확인 이후의 침묵이라 이미 여러 차례 확인된 상태(새 이상
아님). `cloud4c.md`(15.6일)는 `COMMS.md` 등록부에 "⚪ 완료"로 이미
은퇴 처리된 임시 기기라 경고 대상이 아니다. `external16.md`는
`git log --all` 로도 이 경로를 건드린 커밋이 없다 — 등록부에도 "⚪
완료"로 이미 은퇴돼 있어 "무응답"이 아니라 "우편함이 애초에 없음".
...
**이상 없음** — 27개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
09-19 감사(25개) 대비 `v3_humid_wc/` 에 파일 2개(`humid_working_capacity_
mslm050.json`, `humid_working_capacity_nb3.json`)가 새로 늘었다 —
새 결과 반입으로 보이며 이상이 아니다.

### 4) 출처 규약 위반

`merge_water_batches.py`의 `machine_of()`(태그 없는 `water_results.json`
을 `SystemExit` 으로 거부하는 코드를 직접 확인, `main()` 은 실행 안 함)
을 기준으로, 태그 파일 4개(`v3_water_grid/water_results_desktop4.json`,
`water_results_laptop.json`, `water_results_junseok.json`,
`v3_water_grid_cliff/water_results_desktopcliff.json`) 사이에서
(name, RH) 조합이 겹치는지, 그리고 태그 없는 두 `water_results.json`
사본과 값이 완전히 같은지 직접 대조했다.

**경고(중복, 08-23 유형): 태그 없는 사본 2개가 태그 파일과 행 단위로
완전히 동일하다.**
- `v3_water_grid/water_results.json` (미태그, 4행, `saIm0583` RH
  0/0.25/0.5/0.9) == `v3_water_grid/water_results_desktop4.json` —
  4행 전부 `CO2_molkg`·`H2O_molkg`·오차까지 바이트 단위로 동일.
- `v3_water_grid_cliff/water_results.json` (미태그, 4행, `saIm0917`·
  `saIm0958` RH 0/0.9) == `v3_water_grid_cliff/water_results_
  desktopcliff.json` — 4행 전부 동일.
...
않는다 — 지금 저장소 상태에서 활성 이중 계수는 확인되지 않았다. 다만
미태그 사본이 디렉터리에 남아 있는 것 자체가 "글롭으로 두 파일을 같이
넘기면 즉시 재발" 하는 구조적 위험이라 그대로 적어 둔다.

**진짜 교차검증(중복 아님)**: `saIm0875` 가 `water_results_junseok.json`
과 `water_results_laptop.json` 양쪽에 **다른 값**으로 존재한다 — 두
기기가 같은 조성을 독립으로 쟀다.
...
`v3_water_grid*/` 의 태그 파일(및 태그 파일과 값이 동일함이 §4 에서
확인된 미태그 사본)에서 조성별 RH0→RH0.9 의 `CO2_molkg` 비(파일 자체의
`CO2_retention_pct` 필드와 대조 후 재계산)로 관문(≥80% 유효 / 50~80%
조건부 / <50% 종료)을 다시 적용했다.
...
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.06% | 조건부 | `COMMS/desktop.md`("75.1%, 조건부") 일치(반올림差 0.04p) |
| `saIm0875` | junseok | 1.2905 | 0.9619 | 74.54% | 조건부 | `COMMS/junseok.md`·`desktop.md`("74.5%, 조건부") 일치 |
| `saIm0875` | laptop | 1.3031 | 0.9671 | 74.21% | 조건부 | 별도 명시 인용 없음(§4 교차검증 값, 버킷은 동일) |
| `saIm0917` | desktopcliff/미태그 | 1.3249 | 0.9725 | 73.41% | 조건부 | 09-19 감사 표("73.41%, 조건부") 일치 |
| `saIm0958` | desktopcliff/미태그 | 1.5205 | 1.0032 | 65.98% | 조건부 | `COMMS/desktop.md`("ret90(saIm0958)=65.98") 값 일치 |

**이상 없음** — `v3_water_grid*/` 의 7개 (조성·기기) 조합 전부 50~80%
구간(조건부)이고, 값과 관문 판정 모두 저장소 문서와 어긋나지 않는다.
(참고: `saIm0958` 를 "62.6~62.7%" 로 적은 문서들(`MAGI-004_balthasar_
laptop2.md`, `ASSIGN_20260907.md` 등)은 이 파일이 아니라 laptop2 의
별도 실현·9조각 창 평균을 인용한 것으로, `RH90_DENOMINATORS_20260906.md`
가 이미 "`v3_water_grid_cliff/water_results.json` 단일" 이라고 출처를
구분해 뒀다 — 모순이 아니라 서로 다른 측정치.)

### 6) WC 자료 공백

`regen_energy_v3.py` 소스를 직접 읽었다(실행하지 않음). `NAMES` 는
`dry_tsa`·`dry_vsa`·`wet_tsa`·`wet_vsa` 4개 키와 `Qst_CO2` 가 **모두
있는** 조성만 남기도록(67~69행) 이미 걸려 있다 — 08-27 감사가 잡았던
"건조·습윤 둘 다 있는데 파일 목록에서 빠짐" 결함은 그 이후 글롭 방식으로
수정된 채 유지되고 있다(23~32행 주석). `main()` 을 실제로 돌리지 않고
`v3_wc/`·`v3_humid_wc/`·`v4_humid_wc/` 의 `name` 목록만 직접 집계했다.
...
- **건조·습윤 둘 다 있음(7)**: `base`, `mslm075`, `saIm025`, `saIm050`,
  `saIm0583`, `saIm075`, `saIm100` — `regen_energy_v3.json` 에 실제로
  행이 생기는 조성.
- **습윤만 있음(8)**: `mslm025`, `mslm050`, `ms50nb50`, `nbIm075`,
  `nbIm100`, `sa25nb75`, `sa50nb50`, `saIm0625` — 건조 WC 미등록이라
  `NAMES` 에서 자동 제외됨(코드로 확인, 절반만 찬 행이 만들어지지 않음).
- **건조만 있음(0)**: 없음.

**이상 없음** — 필터가 코드로 걸려 있어 지금 당장 "절반만 찬 행"이
만들어지지는 않는다. 다만 습윤만 있는 8개 조성(특히 route-B 혼합
`ms50nb50`·`sa25nb75`·`sa50nb50`) 은 건조 WC 가 등록되기 전까지
`regen_energy_v3.json` 에 아예 나타나지 않는다 — 사람이 그 출력만 보면
"이 조성들은 재생에너지가 나쁘다" 가 아니라 "아직 안 실렸다" 로 읽어야
한다.
...
- 경고: `laptop-20260822` 가 master 대비 **25커밋** 뒤처짐(문턱 24 초과,
  단 마지막 커밋 9분 전 — 활동 중단 아님).
- 경고(반복, 새 이상 아님): `junseok-20260822` 는 22.35일째 무갱신·
  1095커밋 뒤처짐이나 `merge-base` 가 자기 끝점이라 이미 전부 `master`
  에 흡수·은퇴된 상태(08-29 종결 확인).
- 참고(경고는 아님): `v3_water_grid*/` 의 미태그 `water_results.json`
  2개가 태그 파일과 행 단위로 완전히 동일 — `machine_of()` 가 막고
  있어 지금은 안전하지만 글롭으로 같이 넘기면 08-23 유형 이중 계수가
  재발하는 구조.

---

## 2026-09-19 00:09 UTC — 일일 감사

**실행 조건**: 클라우드 저장소 체크아웃만으로 확인. 로컬 프로세스·메모리·
미푸시 커밋은 볼 수 없음. 새 계산 없음 — `merge_water_batches.py`는 태그
붙은 파일만 그대로 실행(글롭 아님), `regen_energy_v3.py`는 모듈로 임포트해
`NAMES`만 조회(`main()` 미실행). 아래 6개 점검 외 어떤 파일도 수정하지
않음.

**방법 메모(경고 아님, 반복되는 검사기 함정)**: 이번 체크아웃도 얕은
클론이었다(`git rev-parse --is-shallow-repository` → `true`). `git fetch
--unshallow` 로 정정 후 아래 1절 수치를 냈다 — 정정 전(얕은 클론) 상태의
`rev-list --left-right`는 laptop 앞섬을 1271, junseok 앞섬을 409로
잘못 부풀렸다(공통 조상 계산이 얕은 히스토리에 갇힘 — 이 저장소가
반복해서 데인 "실패가 결과처럼 보이는 것" 유형과 같은 모양이라 쓰지
않았다). 아래는 전부 `--unshallow` 이후 `git merge-base` 기반 값이다.

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-19
00:09 UTC = 09-19 09:09 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `fcd587d` 2026-09-18 18:02:43 UTC (09-19 03:02:43 KST) | 6.1시간 | — | — |
| `origin/laptop-20260822` | `972d9a4` 2026-09-18 01:17:28 UTC (09-18 10:17:28 KST) | 22.9시간 | **35커밋** | 39커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (2026-08-29 00:54:23 KST) | **512.2시간(21.34일)** | **932커밋** | 0커밋 |

**경고: `laptop-20260822`가 master 대비 35커밋 뒤처짐 — 08-24 사고 기준
(24커밋)을 계속 초과 중이며 09-18 감사의 31커밋보다 더 벌어졌다.**
`git merge-base`는 `65e1ee1`로 09-18 감사와 동일한 조상을 반환해(강제
푸시 아님) laptop 쪽 자기 커밋(앞섬 38→39, +1)은 계속 늘고 있다 — 마지막
커밋이 22.9시간 전으로 활동 중단은 아니다. 다만 그 사이 master가 계속
전진하는데 laptop이 그 커밋들을 받지 않아 뒤처짐이 매일 벌어지는
패턴이다. **지시된 문턱을 그대로 적용해 경고로 올린다.**

**경고(지속, 09-09 감사부터 반복): `junseok-20260822`가 마지막 커밋
21.34일 경과, master 대비 932커밋 뒤처짐** — 08-24 사고 기준의 38.8배.
`merge-base`가 `41fb1b7`(junseok 자기 브랜치 끝점) 그 자체를 반환하고
"앞섬"이 0커밋이라, 이 브랜치 내용은 이미 전부 `master`에 흡수돼 있다
— 08-29 자진 종결 확인(`COMMS/junseok.md` "가지 취합 종결 확인") 이후
새 활동이 없는 것으로 09-10~09-18 감사가 반복 확인한 것과 같은 상태다.
08-24 유형의 "놓친 뒤처짐" 사고는 아니지만, 지시된 문턱(24커밋)을 그대로
적용해 경고로 남긴다.

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825`: 마지막 커밋
`0ad4e56` 2026-09-18 18:03:30 UTC(6.1시간 전), master와 `merge-base`
`4ee4f2c` 부근(정상 분기, 확인만 하고 표에는 안 넣음). 그 외
`origin/junseok`(접미사 없음, 마지막 커밋 2026-08-29 02:32:05 UTC)·
`origin/magi004-*` 3종·`origin/claude/essential-programs-install-ur7v6e`도
존재하나 지시된 점검 대상이 아니다.

### 2) 우편함 침묵

`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋) 기준, 기준
시각 2026-09-19 00:09 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 소속 브랜치 | 경과 |
|---|---|---|---|
| `COMMS/desktop.md` | `fb55aea` 2026-09-17 03:53:40 UTC | master(외 다수) | 44.3시간(1.84일) |
| `COMMS/laptop.md` | `972d9a4` 2026-09-18 01:17:28 UTC | **laptop-20260822 전용** | 22.9시간 |
| `COMMS/laptop2.md` | `0ad4e56` 2026-09-18 18:03:30 UTC | **laptop2-20260825 전용** | 6.1시간 |
| `COMMS/junseok.md` | `be2bede` 2026-08-29 02:32:05 UTC | master(외 다수) | 501.6시간(20.90일) |
| `COMMS/cloud4c.md` | `84cfdde` 2026-09-04 08:49:13 UTC | master(외 다수) | 351.3시간(14.64일) |
| `COMMS/external16.md` | (저장소에 파일 없음) | — | — |
| `COMMS/audit.md` | `fb61707` 2026-09-18 00:11:57 UTC | master(외 다수) | 24.0시간 |

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. **이상
없음** — `desktop.md`·`laptop.md`·`laptop2.md`는 각자 최근 활동(1.84일
이내)으로 정상. `junseok.md`(20.90일)는 1절의 `junseok-20260822` 정체와
같은 방향으로, 08-29 자진 종결 확인 이후의 침묵으로 이미 반복
확인됨(새로운 이상 아님). `cloud4c.md`(14.64일)는 은퇴된 임시 기기
(`COMMS.md` 등록부 "⚪ 완료")라 경고 대상이 아니다. `external16.md`는
`git ls-tree -r --all`로 확인해도 저장소에 커밋된 적이 없는 파일이다 —
`COMMS.md` 등록부에도 이 기기는 "⚪ 완료"로 이미 은퇴 처리돼 있어
"무응답"이 아니라 "우편함이 애초에 만들어지지 않음"이다.

### 3) 결과 JSON 무결성

`results_v3*.json`(10개)·`v3_wc/`(2개)·`v3_humid_wc/`(5개)·
`v4_humid_wc/`(2개)·`v3_water_grid/`(4개)·`v3_water_grid_cliff/`(2개) —
총 25개 JSON을 전부 파이썬 `json.load`로 파싱하고 (dict 형태는) `rows`
필드를 펴서, (list 형태는) 리스트 자체의 길이를 셌다.

`results_v3.json`(31행)·`results_v3_smoke.json`(1)·`results_v3cliff.json`(2)·
`results_v3ens0583.json`(5)·`results_v3ens075.json`(5)·
`results_v3ens_mslm050.json`(5)·`results_v3ens_mix10.json`(10)·
`results_v3ens_nb050.json`(5)·`results_v3grid.json`(4)·
`results_v3pctl.json`(5)·`v3_wc/working_capacity.json`(6)·
`v3_wc/working_capacity_g0583.json`(1)·
`v3_humid_wc/humid_working_capacity.json`(4)·
...
**이상 없음** — 25개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
09-18 감사와 파일 구성·행수가 완전히 동일(그 사이 새 결과 JSON 없음).

### 4) 출처 규약 위반

`merge_water_batches.py`의 `machine_of()`(태그 없는 `water_results.json`은
`SystemExit`으로 거부하는 코드를 직접 확인)를 기준으로, 태그 파일 4개
(`v3_water_grid/water_results_desktop4.json`, `water_results_laptop.json`,
`water_results_junseok.json`, `v3_water_grid_cliff/water_results_desktopcliff.json`)
사이에서 (name, RH) 조합이 두 파일 이상에 겹치는지 직접 대조.

**이상 없음** — 교차 검증 쌍 1개를 찾았다: `saIm0875`가 `junseok`·`laptop`
두 태그 파일 모두에 RH 0/0.25/0.5/0.9 네 점으로 있는데 **값이 전부
다르다**(RH0 CO2 1.2905 대 1.3031, RH0.9 CO2 0.9619 대 0.9671) — 값이
완전히 같은데 두 기기로 잘못 셈한 08-23형 사고가 아니라 **진짜 독립
재현**이다.

참고로 `v3_water_grid/water_results.json`(태그 없음, `saIm0583` 4행)은
`water_results_desktop4.json`과 `diff -q`로 바이트 단위 완전히 동일,
`v3_water_grid_cliff/water_results.json`도 `water_results_desktopcliff.json`과
완전히 동일함을 직접 확인 — 러너가 이어받기 상태 파일과 기기 태그
사본을 같이 쓰는 알려진 부산물이며, `machine_of()`가 태그 없는 파일명을
구조적으로 거부하므로 도구를 통해 병합하는 한 이중 계수로 이어지지
않는다. 나머지 조성(`saIm0583`/`saIm0625`/`saIm0667`/`saIm0917`/
`saIm0958`)은 각각 1개 태그 파일에만 있어 단일 출처. 09-12·09-17·09-18
감사와 동일한 결론.

### 5) 사전 등록 관문 대비 기록

태그 파일의 RH0·RH90 `CO2_molkg`로 유지율(=RH90/RH0×100, 문턱 ≥80%
유효/50~80% 조건부/<50% 종료)을 직접 재계산해 `COMMS/desktop.md`·
`COMMS/laptop.md`·`COMMS/junseok.md`에 적힌 기존 판정과 대조:

| 조성 | 출처 | RH0 CO2 | RH90 CO2 | 유지율 | 관문 재판정 | 저장소 기록 | 일치 |
|---|---|---|---|---|---|---|---|
| `saIm0583` | desktop4 | 1.3188 | 1.0010 | 75.90% | 조건부 | `desktop.md`·`junseok.md` 75.9% 조건부 | ✅ |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.74% | 조건부 | `laptop.md` 71.7% 조건부 | ✅ |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.06% | 조건부 | `desktop.md` 75.055~75.1% 조건부 | ✅ |
| `saIm0875` | junseok | 1.2905 | 0.9619 | 74.54% | 조건부 | `desktop.md`·`junseok.md` 74.5% 조건부 | ✅ |
| `saIm0875` | laptop | 1.3031 | 0.9671 | 74.21% | 조건부 | `laptop.md` 74.2% 조건부 | ✅ |
| `saIm0917` | desktopcliff | 1.3249 | 0.9725 | 73.41% | 조건부 | `desktop.md`("절벽 유지율 0917 73.4") | ✅ |
| `saIm0958` | desktopcliff | 1.5205 | 1.0032 | 65.98% | 조건부 | `desktop.md`("ret90(saIm0958)=65.98") | ✅ |

**이상 없음** — 6조성(교차 검증 쌍 `saIm0875` 포함 7행) 전부 관문
재판정과 저장소 기록이 일치. 어긋난 곳 없음.

- **참고(지속, 새 경고 아님 — 09-09 감사부터 최소 10일째 미해결)**: 위
  표 전체가 `v3_water_grid*/`(2026-09-06 물 힘장 수정 **이전**의 결함판
  — 파일 자체에 `WARN_forcefield` 태그가 RH>0 행마다 붙어 있음, 직접
  확인함)에서 나왔다. 수정 힘장 계열(`v3w_water*/`)이 이 감사 지시
  범위(`v3_water_grid*/`) 밖이라 위 표에는 넣지 않았다 — 이 캐비어트는
  09-09 감사부터 이미 추적 중인 사항이고 오늘 새로 발견된 것이 아니다.

### 6) WC 자료 공백

`v3_wc/*.json`(건조)·`v3_humid_wc/*.json`+`v4_humid_wc/*.json`(습윤)의
모든 `rows[].name`을 모아 조성 집합을 비교하고, `regen_energy_v3.py`를
모듈로 임포트해 실제 `NAMES`(코드 확인: 건조 tsa/vsa·습윤 tsa/vsa·Q_st
다섯 값이 **전부 있는** 조성만 채택 — `regen_energy_v3.py:66-69`)도
대조:
...
**이상 없음** — 건조만 있고 습윤이 없는 조성은 없음. 습윤만 있는 조성
4종이 있으나 `regen_energy_v3.py`의 `NAMES`(모듈 임포트로 직접 확인)가
`['base', 'mslm075', 'saIm025', 'saIm050', 'saIm0583', 'saIm075',
'saIm100']`로 이 4종을 정확히 제외하고 있어, 절반만 찬 행이 완성된
행처럼 계산되는 사고는 재현되지 않는다. 09-12·09-17·09-18 감사와 동일.

## 사람이 볼 것

- **경고**: `laptop-20260822`가 master 대비 35커밋 뒤처짐(24커밋 문턱
  초과, 09-18의 31커밋보다 더 벌어짐 — laptop 자체는 22.9시간 전까지
  활동 중이었으나 master 전진분을 받지 않고 있음).
- **경고(지속, 09-09 감사부터)**: `junseok-20260822`가 21.34일째
  무갱신, master 대비 932커밋 뒤처짐 — 08-29 자진 종결 확인 이후의
  알려진 상태(브랜치 내용은 이미 master에 흡수됨).
- 그 외 4건(우편함 침묵·JSON 무결성·출처 중복·유지율 관문·WC 자료
  공백)은 전부 이상 없음.

---

## 2026-09-18 00:09 UTC — 일일 감사
...
**방법 메모(경고 아님, 반복되는 검사기 함정)**: 이번 체크아웃도 얕은
클론이었다(`git rev-parse --is-shallow-repository` → `true`). `git fetch
--unshallow` 로 정정 후 아래 1절 수치를 냈다 — 얕은 클론 상태의
`rev-list`/`merge-base` 결과는 이 저장소가 반복해서 데인 "실패가
결과처럼 보이는 것" 유형을 그대로 재현하므로 쓰지 않았다.

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-18
00:09 UTC = 09-18 09:09 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `3c81d1a` 2026-09-17 20:29:28 UTC (09-18 05:29:28 KST) | 3.7시간 | — | — |
| `origin/laptop-20260822` | `00eec8a` 2026-09-16 20:35:36 UTC (09-17 05:35:36 KST) | 27.6시간 | **31커밋** | 38커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (2026-08-29 00:54:23 KST) | **488.3시간(20.34일)** | **928커밋** | 0커밋 |

**경고: `laptop-20260822`가 master 대비 31커밋 뒤처짐 — 08-24 사고 기준
(24커밋)을 오늘 처음 넘었다.** 09-17 감사 때는 15커밋(문턱 아래)이었다.
`git merge-base`는 `65e1ee1`로 그제와 동일한 정상 조상을 반환했고(강제
푸시 아님), 원인은 **laptop 쪽 정체가 아니라 master 쪽 전진**이다 —
laptop 의 마지막 커밋 해시(`00eec8a`)가 09-17 감사 기록과 완전히 같고,
그 사이 `master`가 `e86f98d`→`3c81d1a`로 정확히 16커밋 나아갔다
(`git rev-list --count e86f98d..3c81d1a` = 16, 뒤처짐 증가분 15→31의
+16과 일치). laptop 의 앞섬(38커밋)도 09-17과 동일해 laptop 자신은
새 커밋을 만들지 않았을 뿐 활동 중단은 아니다(자기 브랜치 최신 커밋이
27.6시간 전 — 계산 소요 감안 시 정상 범위). **다만 지시된 문턱은
그대로 적용해 경고로 올린다** — 앞으로도 laptop 이 master 를 안 받으면
뒤처짐은 계속 벌어진다.

**경고(지속, 09-09 감사부터 반복): `junseok-20260822`가 마지막 커밋
20.34일 경과, master 대비 928커밋 뒤처짐** — 08-24 사고 기준의 38.7배.
`merge-base`가 `41fb1b7`(junseok 자기 브랜치 끝점) 그 자체를 반환하고
"앞섬"이 0커밋이라, 이 브랜치 내용은 이미 전부 `master`에 흡수돼 있다
— 08-29 자진 종결 확인 이후 새 활동이 없는 것으로 09-10~09-17 감사가
반복 확인한 것과 같은 상태. 08-24 유형의 "놓친 뒤처짐" 사고는 아니지만,
지시된 문턱(24커밋)을 그대로 적용해 경고로 남긴다.

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825`도 확인함 —
마지막 커밋 `49d9f90` 2026-09-17 20:30:27 UTC(3.6시간 전), master 와
`merge-base` `4ee4f2c`(정상 분기, 확인만 하고 표에는 안 넣음). 그 외
`origin/junseok`(접미사 없음)·`origin/magi004-*` 3종도 존재하나 지시된
점검 대상이 아니다.

### 2) 우편함 침묵

`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋) 기준, 기준
시각 2026-09-18 00:09 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 소속 브랜치 | 경과 |
|---|---|---|---|
| `COMMS/desktop.md` | `fb55aea` 2026-09-17 03:53:40 UTC | master (외 다수) | 20.2시간(0.84일) |
| `COMMS/laptop.md` | `00eec8a` 2026-09-16 20:35:36 UTC | **laptop-20260822 전용** | 27.6시간(1.15일) |
| `COMMS/laptop2.md` | `49d9f90` 2026-09-17 20:30:27 UTC | **laptop2-20260825 전용** | 3.6시간(0.15일) |
| `COMMS/junseok.md` | (2026-09-12 이후 갱신 없음, `56d1a38` 기준) | master (외 다수) | 149.2시간(6.22일)\* |
| `COMMS/cloud4c.md` | (2026-09-12 이후 갱신 없음, `56d1a38` 기준) | master (외 다수) | 149.2시간(6.22일)\* |
| `COMMS/audit.md` | `77e0c5f` 2026-09-17 00:13:27 UTC | master (외 다수) | 23.9시간 |

\* `junseok.md`·`cloud4c.md`는 최근 값을 두 개 이상의 다른 파일이
공통으로 건드린 하나의 병합 커밋(`56d1a38`, "E(sa50nb50e1·e2) 완주
JSON 기록")에서 마지막으로 갱신됐다 — 그 커밋이 두 파일을 같이
스쳤을 뿐, 우편함 내용 자체를 갱신한 것은 아닐 수 있다(본문 갱신 여부는
이 점검 범위 밖).

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. **이상
없음** — `desktop.md`는 09-17 감사가 올린 6.66일 침묵 경고를 20.2시간
전 커밋(`fb55aea`, 커밋 메시지 자체가 "우편함 침묵 해소"를 명시)으로
직접 해소했다(git log 로 확인: 저장소 파일만 보고 판단, 그 커밋
메시지의 자기 신고를 그대로 인용). `laptop.md`·`laptop2.md`는 각 기기
자기 브랜치 기준 하루 이내로 정상. `cloud4c.md`(은퇴된 임시 기기,
경고 아님)와 `junseok.md`(1절의 `junseok-20260822` 정체와 같은 방향,
자발적 종결 이후 침묵으로 이미 확인됨)는 새로운 이상이 아니다.
...
`v4_humid_wc/`(2개), `v3_water_grid/`(4개), `v3_water_grid_cliff/`(2개) —
총 25개 JSON을 전부 파이썬 `json.load`로 파싱하고 (dict 형태는) `rows`
필드를 펴서, (list 형태는) 리스트 자체의 길이를 셌다(최초에 top-level
dict 키 개수만 셌다가 전부 "4행"으로 잘못 나오는 것을 발견 —
`source`/`conditions`/`rows`/`WARN_qst_rt` 등 메타 키였고, 실제 데이터는
`rows` 필드 안에 있었다. 09-17 감사가 기록한 것과 같은 검사기 함정).
...
**이상 없음** — 25개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
09-17 감사와 파일 구성·행수가 완전히 동일(그 사이 새 결과 JSON 없음).

### 4) 출처 규약 위반

`merge_water_batches.py`를 태그 파일 4개(`v3_water_grid/water_results_desktop4.json`,
`water_results_laptop.json`, `water_results_junseok.json`,
`v3_water_grid_cliff/water_results_desktopcliff.json`)에 직접 실행(글롭
아님 — 도구 자신의 `machine_of()`가 태그 없는 `water_results.json`은
`SystemExit`으로 거부).

**이상 없음** — 도구가 교차 검증 쌍 4개를 찾았다: `saIm0875`가
`junseok`·`laptop` 두 태그 파일 모두에 RH 0/25/50/90 네 점으로 있는데
값이 전부 다르다(RH0 CO2 1.2905 vs 1.3031, 편차 0.32σ/0.75σ/0.03σ/0.10σ
— 도구 출력 그대로 인용). 전부 등록 문턱(<2.0σ) 안 "일관" — 값이 완전히
같은데 두 기기로 잘못 셈한 08-23형 사고가 아니라 **진짜 독립 재현**이다.
...
구조적으로 거부하므로 도구를 통해 병합하는 한 이중 계수로 이어지지
않는다. 나머지 조성(`saIm0583`/`saIm0625`/`saIm0667`/`saIm0917`/
`saIm0958`)은 각각 1개 태그 파일에만 있어 단일 출처. 09-12·09-17 감사와
동일한 결론.

### 5) 사전 등록 관문 대비 기록

`merge_water_batches.py`가 낸 관문 표(RH0·RH90 `CO2_molkg`로 유지율 =
RH90/RH0×100, 문턱 ≥80% 유효/50~80% 조건부/<50% 종료)와 `COMMS/desktop.md`·
`COMMS/laptop.md`·`COMMS/junseok.md`에 적힌 기존 판정을 대조:

| 조성 | 출처 | RH0 CO2 | RH90 CO2 | 유지율 | 관문 재판정 | 저장소 기록 | 일치 |
|---|---|---|---|---|---|---|---|
| `saIm0583` | desktop4 | 1.3188 | 1.0010 | 75.9% | 조건부 | `COMMS/desktop.md`·`junseok.md` 75.9% 조건부 | ✅ |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.7% | 조건부 | `COMMS/laptop.md` 71.7% 조건부 | ✅ |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.1% | 조건부 | `COMMS/desktop.md` 75.055~75.1% 조건부 | ✅ |
| `saIm0875` | junseok\* | 1.2905 | 0.9619 | 74.5% | 조건부 | `COMMS/desktop.md`·`junseok.md` 74.5% 조건부 | ✅ |
| `saIm0917` | desktopcliff | 1.3249 | 0.9725 | 73.4% | 조건부 | `COMMS/desktop.md`("절벽 유지율 0917 73.4") | ✅ |
| `saIm0958` | desktopcliff | 1.5205 | 1.0032 | 66.0% | 조건부 | `COMMS/desktop.md`("ret90(saIm0958)=65.98") | ✅ |

\* `saIm0875`는 4절의 교차 검증 쌍이고, laptop 값(74.2%)도 동일하게 조건부.

**이상 없음** — 6조성 전부 관문 재판정과 저장소 기록이 일치. 어긋난
곳 없음.

- **참고(지속, 새 경고 아님 — 09-09 감사부터 최소 9일째 미해결)**: 위
  표 전체가 `v3_water_grid*/`(2026-09-06 물 힘장 수정 **이전**의 결함판
  — 파일 자체에 `WARN_forcefield` 태그가 RH>0 행마다 붙어 있음, 직접
  확인함)에서 나왔다. 수정 힘장 계열(`v3w_water/` 등)이 이 감사 지시
  범위(`v3_water_grid*/`) 밖이라 위 표에는 넣지 않았다 — 다만 09-17
  감사가 기록한 대로 그쪽 값으로 재계산하면 일부 조성(`saIm0583`·
  `saIm0625`)이 조건부→유효로 갈래가 바뀔 수 있는데 공식 문서 갱신
  여부는 이번 체크아웃에서 새로 확인하지 못했다(범위 밖).

### 6) WC 자료 공백

`v3_wc/*.json`(건조)·`v3_humid_wc/*.json`+`v4_humid_wc/*.json`(습윤)의
모든 `rows[].name`을 모아 조성 집합을 비교하고, `regen_energy_v3.py`를
모듈로 임포트해 실제 `NAMES`도 대조:
...
`['base', 'mslm075', 'saIm025', 'saIm050', 'saIm0583', 'saIm075',
'saIm100']`로 이 4종을 정확히 제외하고 있어, 절반만 찬 행이 완성된
행처럼 계산되는 사고는 재현되지 않는다. 09-12·09-17 감사와 동일.

## 사람이 볼 것

1. **`laptop-20260822`가 오늘 처음 master 대비 24커밋 문턱을 넘었다
   (31커밋)** — laptop 자체 정체가 아니라 master 가 16커밋 전진한
   것이 원인(laptop 마지막 커밋은 09-17 감사와 동일 해시). laptop 이
   다음에 master 를 받을 때 반영 확인 필요.
2. `junseok-20260822` 20.34일/928커밋 정체 — 이미 master에 흡수(0
   앞섬), 08-29 자진 종결 확인됨. 지속 경고로만 유지.
3. 5절 관문 판정은 자료와 일치하나 그 기반이 물 힘장 수정 이전
   (`WARN_forcefield`) 값이라는 점이 9일째 미해결 — 공식 재등급 여부
   확인 필요.

---

## 2026-09-17 00:11 UTC — 일일 감사
...
**공백 고지**: 이 로그의 직전 항목은 2026-09-12 00:12 UTC — 그 사이
09-13~09-16 나흘치 자동 감사 항목이 없다(이 세션이 그 날짜들을 대신
채우지는 않음, 사실이 아닌 것을 적을 수 없어서). 아래는 09-17 시점의
직접 재확인이며 "어제 대비"라는 표현은 실제로는 5일 전 대비다.

**방법 메모(경고 아님, 이번에도 재현된 검사기 함정)**: 컨테이너 체크아웃이
또 얕은 클론이었다(`git rev-parse --is-shallow-repository` → `true`,
09-06~09-12 감사가 반복 기록한 것과 같음). unshallow 전 `git rev-list
--left-right --count origin/master...origin/laptop-20260822`는 "15
1270"으로, `origin/junseok-20260822`는 `merge-base`가 공통 조상을 못 찾고
rc=1을 반환해 마치 master와 아예 다른 뿌리에서 갈라진 것처럼 보였다.
`git fetch --unshallow` 후 재계산하니 laptop은 "15 38"로(뒤처짐 15는
같았으나 앞섬이 1270→38로 32배 정정), junseok은 `merge-base`가
`41fb1b7`(junseok 자기 끝점)을 정상 반환해 "912 0"으로 나왔다. **아래
1절 수치는 전부 unshallow 이후 값이다** — 얕은 클론 상태에서의 큰
"앞섬" 수치나 merge-base 실패만 보고 브랜치가 강제 푸시로 분리됐다고
단정하면 안 된다(이 프로젝트가 반복해서 데인 "실패가 결과처럼 보이는
것" 유형, 09-09·09-12 감사와 동일 교훈).

### 1) 브랜치 정체

`git fetch --all` + `git fetch --unshallow` 후 (기준 시각 2026-09-17
00:11 UTC = 09:11 KST):

| 브랜치 | 마지막 커밋 | 경과 | master 대비 뒤처짐 | master 대비 앞섬 |
|---|---|---|---|---|
| `origin/master` | `e86f98d` 2026-09-16 20:56:36 UTC (09-17 05:56:36 KST) | 3.2시간 | — | — |
| `origin/laptop-20260822` | `00eec8a` 2026-09-16 20:35:36 UTC (09-17 05:35:36 KST) | 3.6시간 | 15커밋 | 38커밋 |
| `origin/junseok-20260822` | `41fb1b7` 2026-08-28 15:54:23 UTC (2026-08-29 00:54:23 KST) | **464.3시간(19.34일)** | **912커밋** | 0커밋 |

`laptop-20260822`: master 대비 15커밋 뒤처짐 — 08-24 사고 기준(24커밋)
아래, **경고 아님**. `git merge-base`는 `65e1ee1`로 정상 조상을 찾았고
(정상 분기), 최근 커밋이 3.6시간 전으로 활발히 진행 중.

**경고: `junseok-20260822`가 마지막 커밋 19.34일 경과, master 대비
912커밋 뒤처짐** — 08-24 사고 기준의 38.0배, 09-12 감사(14.34일/885커밋)
대비 5.0일·27커밋 더 벌어짐(그 사이 감사가 4일 빠진 것과 대략 일치).
다만 `merge-base`가 `41fb1b7`(junseok 자기 브랜치 끝점) 그 자체를
반환했고 "앞섬"이 0커밋이라, junseok의 내용은 이미 전부 `master`에
흡수돼 있다 — 09-12 감사가 확인한 것과 같은 상태(2026-08-29 "가지
취합 종결 확인" 커밋으로 스스로 접힌 뒤 새 활동 없음). 08-24 유형의
"놓친 뒤처짐" 사고는 아니지만, **지시된 문턱(24커밋)은 그대로 적용해
경고로 남긴다.**

(참고, 지시된 세 브랜치 밖) `origin/laptop2-20260825`·`origin/junseok`
(접미사 없음)·`origin/magi004-*` 3종도 존재하나 지시된 점검 대상이 아니라
표에 넣지 않았다.

### 2) 우편함 침묵

`--all`(모든 브랜치 통틀어 그 경로를 마지막으로 건드린 커밋) 기준, 기준
시각 2026-09-17 00:11 UTC:

| 파일 | 마지막 커밋(전체 브랜치) | 소속 브랜치 | 경과 | 참고: `master`만 보면 |
|---|---|---|---|---|
| `COMMS/desktop.md` | `6eb087c` 2026-09-10 08:22:35 UTC | master (외 다수) | **159.8시간(6.66일)** | 동일 |
| `COMMS/laptop.md` | `00eec8a` 2026-09-16 20:35:36 UTC | **laptop-20260822 전용** | 3.6시간(0.15일) | `a30745b` 09-07, **9.93일** |
| `COMMS/laptop2.md` | `282bec9` 2026-09-16 20:57:28 UTC | **laptop2-20260825 전용** | 3.2시간(0.13일) | `558fff3` 09-10, **7.33일** |
| `COMMS/junseok.md` | `be2bede` 2026-08-29 02:32:05 UTC | master (외 다수) | **453.6시간(18.90일)** | 동일 |
| `COMMS/cloud4c.md` | `84cfdde` 2026-09-04 08:49:13 UTC | master (외 다수) | 303.4시간(12.64일) | 동일 |

계산 중이면 침묵이 정상일 수 있어 "죽었다"고 단정하지 않음. `laptop.md`·
`laptop2.md`는 각 기기 브랜치에서 3~4시간 전까지 활발하다 — `master`만
보면 각각 9.93일·7.33일 침묵으로 잘못 보이는데, 이는 09-11 감사가 처음
기록한 것과 같은 "master만 보면 병합 안 된 브랜치 활동을 놓친다"는
함정이 그대로 재현된 것이다(경고 아님). `cloud4c.md`는 이미 은퇴된
임시 기기라 12.64일 침묵이 예상된 상태(경고 아님). `junseok.md`
(18.90일)는 1절의 `junseok-20260822` 정체(19.34일)와 같은 방향으로
겹치고, 자발적 종결 이후의 침묵으로 이미 확인됨(경고로만 유지).

**경고: `COMMS/desktop.md`가 `--all` 기준으로도 6.66일(159.8시간) 침묵** —
다른 두 활성 기기(랩탑·laptop2)가 3~4시간 전까지 커밋했고 이 값은
"master만 봐서 생긴 착시"가 아니다(전체 브랜치 기준으로도 09-10 이후
갱신 없음). 이 저장소 규율(§4)대로 "출력이 며칠째 그대로"라는 사실
자체만으로 "죽었다"고 단정하지 않지만, 최근 커밋 메시지들(§AG·§AI·§AJ
등)에 "데스크탑"이 여러 차례 언급되는데 그 활동이 `COMMS/desktop.md`
갱신으로는 나타나지 않는다 — 우편함이 아니라 다른 경로로 보고되고
있는지, 아니면 실제로 정지했는지는 이 체크아웃만으로 가를 수 없어
사람 확인이 필요하다.

### 3) 결과 JSON 무결성

`results_v3*.json`(10개, 09-12 이후 `results_v3ens_mslm050.json` 1개
추가됨), `v3_wc/`(2개), `v3_humid_wc/`(5개), `v4_humid_wc/`(2개),
`v3_water_grid/`(4개), `v3_water_grid_cliff/`(2개) — 총 25개 JSON을 전부
파이썬 `json.load`로 파싱하고 (dict 형태는) `rows` 필드, (list 형태는)
리스트 자체의 길이를 셌다.

`results_v3.json`(31행)·`results_v3_smoke.json`(1)·`results_v3cliff.json`(2)·
`results_v3ens0583.json`(5)·`results_v3ens075.json`(5)·
`results_v3ens_mslm050.json`(5, 신규)·`results_v3ens_mix10.json`(10)·
`results_v3ens_nb050.json`(5)·`results_v3grid.json`(4)·
`results_v3pctl.json`(5)·`v3_wc/working_capacity.json`(6)·
`v3_wc/working_capacity_g0583.json`(1)·
`v3_humid_wc/humid_working_capacity.json`(4)·
...
**이상 없음** — 25개 전부 파싱 성공, 파싱 실패도 0행인 파일도 없음.
(처음에 top-level dict의 키 개수를 셌다가 `results_v3.json` 등이 전부
"3행"으로 잘못 나온 실수를 발견 — 실제로는 `source`/`conditions`/`rows`
세 키였고, `rows` 필드를 펴서 다시 세어 31행임을 확인했다. 이 자체가
검사기가 틀릴 수 있다는 §0의 교훈을 스스로 겪은 사례라 적어 둔다.)
...
**이상 없음** — 도구가 교차 검증 쌍 4개를 찾았다: `saIm0875`가
`junseok`·`laptop` 두 태그 파일 모두에 RH 0/25/50/90 네 점으로 있는데
값이 전부 다르다(예: RH0 CO2 1.2905 대 1.3031). 편차는 0.32σ·0.75σ·
0.03σ·0.10σ로 전부 등록 문턱(<2.0σ) 안 "일관" — 값이 완전히 같은데
두 기기로 잘못 셈한 08-23형 사고가 아니라 **진짜 독립 재현**이다.

참고로 `v3_water_grid/water_results.json`(태그 없음)은
`water_results_desktop4.json`과, `v3_water_grid_cliff/water_results.json`은
`water_results_desktopcliff.json`과 `diff -q`로 바이트 단위 완전히
동일함을 확인(러너가 이어받기 상태 파일과 기기 태그 사본을 같이 쓰는
알려진 부산물). `machine_of()`가 태그 없는 파일명을 거부하므로 도구를
통해 병합하는 한 이중 계수로 이어지지 않는다. 나머지 조성(`saIm0583`/
`saIm0625`/`saIm0667`/`saIm0917`/`saIm0958`)은 각각 1개 태그 파일에만
있어 단일 출처.
...
| 조성 | 출처 | RH0 CO2 | RH90 CO2 | 유지율 | 관문 재판정 | 저장소 기록 | 일치 |
|---|---|---|---|---|---|---|---|
| `saIm0583` | desktop4 | 1.3188 | 1.0010 | 75.9% | 조건부 | `COMMS/desktop.md` 75.90% 조건부 | ✅ |
| `saIm0625` | laptop | 1.3804 | 0.9902 | 71.7% | 조건부 | `COMMS/laptop.md` 71.7% 조건부 | ✅ |
| `saIm0667` | laptop | 1.3682 | 1.0269 | 75.1% | 조건부 | `COMMS/desktop.md` 75.05~75.1% 조건부 | ✅ |
| `saIm0875` | junseok* | 1.2905 | 0.9619 | 74.5% | 조건부 | `COMMS/junseok.md` 74.5% 조건부 | ✅ |
| `saIm0917` | desktopcliff | 1.3249 | 0.9725 | 73.4% | 조건부 | `COMMS/desktop.md` 73.41% 조건부 | ✅ |
| `saIm0958` | desktopcliff | 1.5205 | 1.0032 | 66.0% | 조건부 | `COMMS/desktop.md` 65.98% 조건부 | ✅ |

\* `saIm0875`는 교차 검증 쌍이 있고(4절), laptop 값(74.2%)도 동일하게
조건부.

**이상 없음** — 6조성 전부 관문 재판정과 저장소 기록이 일치. 어긋난
곳 없음.

- **참고(지속, 새 경고 아님 — 09-09 감사가 처음 올린 뒤 오늘로 최소
  8일째 미해결, 이 감사의 나흘 공백 동안의 진행은 확인 못 함)**: 위
  표 전체가 `v3_water_grid*/`(2026-09-06 물 힘장 수정 **이전**의 결함판,
  파일 자체에 `WARN_forcefield` 태그 있음)에서 나왔다. 수정 힘장 계열
  `v3w_water/water_results.json`을 오늘 다시 확인해도 `saIm0583`(RH90
  CO2 1.1514)·`saIm0625`(1.1477)·`saIm0667`(1.0845) mol/kg으로
  09-09~09-12 감사와 값이 동일하고, 이 값으로 재계산하면 `saIm0583`
  87.3%·`saIm0625` 82~83%는 조건부→유효로 갈래가 바뀐다(과거 감사
  계산). 저장소 전체에서 `조건부→유효`·`재등급`을 검색해도 이 `audit.md`
  자신 외 무매치 — 공식 문서는 아직 갱신되지 않았다. `v3w_water/`는
  이 감사 지시 범위(`v3_water_grid*/`) 밖이라 위 표에는 넣지 않는다.
...
**이상 없음** — 건조만 있고 습윤이 없는 조성은 없음. 습윤만 있는
조성 4종이 있으나 `regen_energy_v3.py`의 `NAMES`(08-27 수정 필터)가
`['base', 'mslm075', 'saIm025', 'saIm050', 'saIm0583', 'saIm075',
'saIm100']`로 이 4종을 정확히 제외하고 있음을 모듈 임포트로 직접
확인했다 — 09-09~09-12 감사와 동일. 절반만 찬 행이 완성된 행처럼
보이는 사고는 현재 재현되지 않는다.

## 사람이 볼 것

1. `junseok-20260822` 19.34일/912커밋 정체 — 이미 master에 흡수(0
   앞섬), 08-29 자진 종결 확인 커밋이 있어 "무응답"은 아니나 문턱
   기준 경고로 유지.
2. **`COMMS/desktop.md` 6.66일 침묵(전체 브랜치 기준)** — 같은 기간
   랩탑·laptop2는 3~4시간 전까지 활발함. 착시가 아니라 실제 값이라
   확인 필요.
3. 5절 관문 판정은 자료와 일치하나 그 기반(`v3_water_grid*/`)이 물
   힘장 수정 이전 값이라는 점이 최소 8일째 미해결(`v3w_water/`가
   `saIm0583`·`saIm0625`를 조건부→유효로 재등급할 수 있는데 공식화
   안 됨) — 이 감사의 나흘 공백 동안 바뀌었을 수도 있음.
