...
[자원 -- 08-12 먹통을 낸 바로 그 조합]
    이완 후 슈퍼셀에 Zeo++ 를 돌리면 한 건이 **9.5 GB** 입니다(08-19 실측).
    08-12 에는 8워커로 OOM 이 났고 dbus 까지 죽어 WSL 이 통째로 멈췄습니다.
    **RASPA 가 돌고 있으면 시작하지 않습니다.**

    [2026-08-22 정정] 이 독스트링은 "한 건 3.2 GB / 기본 워커 4(12.8 GB)" 라고
    적고 있었습니다. 3.2 는 **v1 구조에서 잰 값**이고, 아래 91행 코드는 이미
    9.5 를 쓰고 있었습니다. 같은 파일의 88행이 "문서에서 베끼지 말고 그 기기에서
    재세요" 라고 적어 둔 바로 그 파일에서, 독스트링이 낡은 상수를 가르치고
    있었습니다. 워커 수는 고정값이 아니라 /proc/meminfo 에서 계산합니다.
"""
import json
import os
import shutil
