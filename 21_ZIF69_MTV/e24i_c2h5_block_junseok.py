# -*- coding: utf-8 -*-
"""MAGI-005 E-24i (Junseok) — −C₂H₅ 열린 자리(e24i_c2h5_open, **막음**) 작동점 S_mix 3씨앗. "먼저 띄움"(자격은 Widom 뒤). 등록 ASSIGN_MAGI5B §HKHOME 22차(f5935fdf).
데스크탑 부탁: "e24i_c2h5_open — 주머니가 있으면 E-24e 막음 드라이버 틀로". E-24i Zeo++(이 기기) 1.65 에서 주머니 4 · 구 4.
e24g_c2h5a_block_junseok.py(E-24e 틀, E-24g 후속) 사본 — 바꾼 것: 대상 · 막음 파일(e24i_zeo_runs/<tag>/block{1.65,1.82}) · 실행 폴더 · 출력 이름 · 기록 문구 ·
  기준값 — 막음 Widom 전이라 막음 S_Henry · 물 지수 분모 없음(자리 채움 None · 비 인용 금지), 차단 없음 S_ON 은 Widom 파일이 있으면 서술로 ·
  **물 작업 뺌**(이번 배정은 S_mix 만 — 물 JSON 안 씀). 자(run_tj1_mix 조건) · 성분별 막음 삽입 · 관문 · 씨앗 간격(15 s + 2 s 잠금) · 워커 6 은 그대로.
출력 results_e24i_e24i_c2h5_open_mix_junseok.json(run_e24i_mix.py 와 같은 이름 꼴) · 실행 폴더 e24i_blk_runs_e24i_c2h5_open/. 원판 머리말:
...
ZBASE = os.path.join(HERE, 'e24i_zeo_runs', TAG)                  # E-24g Zeo++(이 기기) — 막음 파일 셋 다 여기
RUNS = os.path.join(HERE, f'e24i_blk_runs_{TAG}')
OUT_MIX = os.path.join(HERE, f'results_e24i_{TAG}_mix_junseok.json')
OUT_W = None                                                       # 물 작업 없음(이번 배정 S_mix 만)
ASSIGN = 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차(f5935fdf) · 데스크탑 부탁(S_mix 먼저 띄움)'
...
UBJ = sorted(glob.glob(os.path.join(HERE, f'results_magi5_e3_{TAG}_widom_*.json')))   # 차단 없음 Widom(있으면) — 서술


def _refs():
    """E-24i 는 막음 Widom 전 '먼저 띄움' — 막음 S_Henry · 물 지수 분모 없음(자리 채움 None, 비 인용 금지). 차단 없음 S_ON 은 파일이 있으면 서술로."""
    u = None
    if UBJ:
        u = [r for r in json.load(open(UBJ[0], encoding='utf-8'))['rows'] if r['charges'] == 'on'][0]['selectivity']
...
def zeo_water():
    """[A] 새로 안 돌림 — E-24i Zeo++(이 기기, results_e24i_access_junseok.json)의 이 대상 1.30 행을 기록만(물 작업이 없어 쓰이지 않음)."""
    row = [r for r in json.load(open(os.path.join(HERE, 'results_e24i_access_junseok.json'), encoding='utf-8'))['rows'] if r['tag'] == TAG][0]
    return {'source': 'results_e24i_access_junseok.json', **row['r1.30']}
...
def summarize(mix, water, zeo, t_start, ffm, wm, final=False):
    ...
    json.dump({'test': f'MAGI-005 E-24i — {TAG}(C₂H₅ 열린 자리) S_mix 3씨앗(성분별 막음, 먼저 띄움)', 'assign': ASSIGN, 'machine': 'junseok',
               'ported_from': 'e24g_c2h5a_block_junseok.py ← magi5_e24e_junseok.py (E-24e)',
               'protocol': 'run_tj1_mix 틀(0.15/0.85 · 1e5 Pa · 298 K · 5,000+15,000) + CO₂ BlockPockets block1.65 · N₂ block1.82 (E-24i Zeo++ e24i_zeo_runs)',
               'reference': {'S_Henry_blocked': None, 'S_Henry_unblocked_descriptive': SH_UNBLOCK,
                             'unblocked_file': os.path.basename(UBJ[0]) if UBJ else None, 'note': '막음 Widom 전 먼저 띄움 — 비 인용 금지(판정 때 원자료로 다시 냄)'},
    ...
    if OUT_W is None:                                       # 물 작업 없음 — 물 JSON 안 씀
        return ms, {}
    wsum = {}
    for r in (1.30,):
    ...
    json.dump({'test': f'MAGI-005 E-24g 후속 — {TAG}(C₂H₅ 50 % a) 물 Widom 4씨앗(막음 1.3)', 'assign': ASSIGN, 'ported_from': 'magi5_e24e_junseok.py (E-24e)',
               ...
               'denominator_KH_CO2_ON_blocked': [KCO2, EKCO2],
...
def main():
    ...
    for r in (1.65, 1.82):
        if not os.path.exists(BLOCK[r]):
            ...
            return 2
    print(f'E-24i {TAG} — CIF {os.path.relpath(CIF, HERE)} · 막음 CO₂ {nsph(1.65)} 구 · N₂ {nsph(1.82)} 구 · 단위셀 {rg.unit_cells(rg.read(CIF))} · '
          f'워커 {WORKERS} · S_mix 3(물 없음) · 막음 S_Henry 없음(먼저 띄움, 비 인용 금지) · 차단 없음 S_ON {SH_UNBLOCK}', flush=True)
    ...
    if n('simulate') not in ('', '0') or n('network') not in ('', '0') or n('lmp_serial') not in ('', '0'):
        ...
        return 1
    for p in (RUNS, OUT_MIX):                               # OUT_W 없음(물 작업 없음)
        if os.path.exists(p):
