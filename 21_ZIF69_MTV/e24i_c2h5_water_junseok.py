# -*- coding: utf-8 -*-
"""MAGI-005 E-24i (Junseok) — −C₂H₅ 열린 자리(e24i_c2h5_open, **막음**) 물 Widom 4씨앗(1.30 막음). 등록 ASSIGN_MAGI5B §HKHOME 22차 끝 줄(27f32f80, 자료 0건).
데스크탑 부탁: "E-24g e24h_c2h5_050a 물과 같게 — 대상만 e24i_c2h5_open, 막음 파일 e24i_zeo_runs/e24i_c2h5_open/block1.30, 출력 results_e24i_c2h5_open_water_blk_junseok.json,
  분모는 비워 두거나 자리 채움(데스크탑 막음 Widom results_e24i_blockpockets_hkhome.json 을 판정 때 종합자가 원자료로 붙임)".
e24g_c2h5a_block_junseok.py(E-24e 틀, E-24g 후속) 사본 — 바꾼 것: 대상 · 막음 파일(e24i_zeo_runs block1.30 구 5) · 실행 폴더 · 출력 이름 · 기록 문구 ·
  분모(K_H(CO₂ ON, 막음 1.65)) 자리 채움 None — 물 지수는 계산 안 함 · **S_mix 작업 뺌**(물 4씨앗만, S_mix JSON 안 씀).
  자(E-24 물 Widom 틀: TIP5P-Ew 5자리 · 15,000 + 3,000 · 298 K · 전하 ON · read_kh(water) · 머리말 관문) · 막음 삽입 · 관문 · 씨앗 간격(15 s + 2 s 잠금) · 워커 6 은 그대로. 원판 머리말:
...
RUNS = os.path.join(HERE, f'e24i_blkw_runs_{TAG}')
OUT_MIX = None                                                     # S_mix 작업 없음(물 4씨앗만)
OUT_W = os.path.join(HERE, 'results_e24i_c2h5_open_water_blk_junseok.json')
ASSIGN = 'ASSIGN_MAGI5B_20260925.md §HKHOME 22차 끝 줄(27f32f80) — 막음 물 4씨앗'
...
def _refs():
    """막음 Widom(데스크탑 results_e24i_blockpockets_hkhome.json) 전 — 물 지수 분모 자리 채움 None(판정 때 종합자가 원자료로 붙임). S_mix 없음."""
...
def zeo_water():
    """[A] 새로 안 돌림 — E-24i Zeo++(이 기기, results_e24i_access_junseok.json)의 이 대상 1.30 행을 기록만(같은 명령 -ha -block 1.30 50000 · -chan 1.30)."""
...
def summarize(mix, water, zeo, t_start, ffm, wm, final=False):
    ...
    for r in (1.30,):
        ...
        wsum[f'block{r:.2f}'] = s
    json.dump({'test': f'MAGI-005 E-24i — {TAG}(C₂H₅ 열린 자리) 물 Widom 4씨앗(막음 1.3)', 'assign': ASSIGN,
               ...
               'denominator_src': '자리 채움 None — 데스크탑 막음 Widom results_e24i_blockpockets_hkhome.json on_CO2@1.65 를 판정 때 종합자가 원자료로 붙임',
...
def main():
    ...
    for r in (1.30,):
        if not os.path.exists(BLOCK[r]):
            ...
            return 2
    print(f'E-24i {TAG} 막음 물 — CIF {os.path.relpath(CIF, HERE)} · 막음 물 {nsph(1.30)} 구 · 단위셀 {rg.unit_cells(rg.read(CIF))} · '
          f'워커 {WORKERS} · 물 1.3 ×4(S_mix 없음) · 분모 자리 채움 None(판정 때 종합자가 원자료로)', flush=True)
    ...
    if n('simulate') not in ('', '0') or n('network') not in ('', '0') or n('lmp_serial') not in ('', '0'):
        ...
        return 1
    for p in (RUNS, OUT_W):                                 # OUT_MIX 없음
        if os.path.exists(p):
    ...
    os.makedirs(RUNS)
    t0 = time.time() + 2
    jobs = [('water', (i, 1.30, k, t0)) for i, k in enumerate((1, 2, 3, 4))]
