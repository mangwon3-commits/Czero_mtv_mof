"""밀도맵 E-28e — E-24i 열린 자리 치환체 건조 CO₂ 격자, 전하 ON/OFF (2026-09-26). 등록 `ASSIGN_MAGI5B_20260925.md` §HKHOME 22차 E-28e.
`run_density_e24g.py` 사본 — 바꾼 것: 출력 폴더 density_v3_tpl4/<SUB> · 막음 파일(DENSITY_E24I_BLOCK 쉼표 목록 → e24i_zeo_runs/<tag>/block1.65/). 원판 첫 줄: 밀도맵 E-28d — E-24g · E-24h 새 치환체 건조 CO₂ 격자.
...
OUTD = os.path.join(REAL, "density_v3_tpl4", os.environ.get("DENSITY_E24G_SUB", ""))
...
BLOCK = {f"{k}_DDEC6": os.path.join(REAL, "e24i_zeo_runs", k, "block1.65", f"{k}_relaxed.block") for k in os.environ.get("DENSITY_E24I_BLOCK", "").split(",") if k and k in rd.TARGETS}
...
if __name__ == "__main__":
    print(f"밀도맵 E-28e 건조 — {rd.TARGETS} · 막음 {list(BLOCK)}", flush=True)
    if not rd.TARGETS:
