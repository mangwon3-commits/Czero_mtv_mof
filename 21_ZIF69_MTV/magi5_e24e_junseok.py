...
ZBASE = os.path.join(HERE, 'e24e_zeo')
RUNS = os.path.join(HERE, 'e24e_runs')
OUT_MIX = os.path.join(HERE, 'results_e24e_c2h5_mix_junseok.json')
OUT_W = os.path.join(HERE, 'results_e24e_c2h5_water_junseok.json')
BLOCK = {1.65: os.path.join(HERE, 'e24c_zeo_runs', TAG, 'block1.65', TAG + '_relaxed.block'),
         1.82: os.path.join(HERE, 'e24c_zeo_runs', TAG, 'block1.82', TAG + '_relaxed.block'),
         1.30: os.path.join(ZBASE, 'block1.30', TAG + '_relaxed.block')}
...
KCO2, EKCO2 = 2.26037e-3, 1.55457e-5                    # E-24c 막음 Widom ON CO₂@1.65
SH_BLOCK, ESH_BLOCK = 188.55586512954835, 4.267419113060723   # E-24c 막음 S_ON
SH_UNBLOCK = 236.40                                      # 서술: 데스크탑 차단 없음 S_ON(results_magi5_e3_e24c_c2h5_100_widom_desktop-js1ib6u.json)
REF_CH3_SMIX, REF_CH3_SMIX_ERR = 128.62, 4.63            # 등록 (1) 기준
REF_PARENT_IDX, REF_PARENT_IDX_ERR = 0.0147, 0.0003      # 등록 (2) 기준
...
def zeo_water():
    """[A] -block 1.3 · -chan 1.3 — 막음 파일을 BLOCK[1.30] 에 남김."""
    res = {}
    for kind, args in (('chan', ['-ha', '-chan', '1.30', f'{TAG}.chan', f'{TAG}_relaxed.cif']),
                       ('block', ['-ha', '-block', '1.30', str(NS), f'{TAG}_relaxed.cif'])):
        d = os.path.join(ZBASE, f'{kind}1.30')
        os.makedirs(d)
        shutil.copy(RELAXED, d)
        rc, sec, peak, stop = run_guarded([ZEO] + args, d, f'zeo_{kind}.out')
        txt = open(os.path.join(d, f'zeo_{kind}.out'), errors='ignore').read()
        m = re.search(r'Identified (\d+) channels and (\d+) pockets', txt)
        r = {'rc': rc, 'seconds': sec, 'peak_rss_MB': peak, 'stopped_by_guard': stop,
             'channels_stdout': int(m.group(1)) if m else None, 'pockets_stdout': int(m.group(2)) if m else None}
        if kind == 'chan':
            c = open(os.path.join(d, f'{TAG}.chan'), errors='ignore').read() if os.path.exists(os.path.join(d, f'{TAG}.chan')) else ''
            mc = re.search(r'(\d+)\s+channels identified of dimensionality\s*([\d\s]*)', c)
            r.update(n_channels=int(mc.group(1)) if mc else None, dimensionality=[int(x) for x in mc.group(2).split()] if mc and mc.group(2).strip() else [])
        else:
            bf = BLOCK[1.30]
            if os.path.exists(bf):
                lines = [ln.split() for ln in open(bf).read().strip().splitlines()]
                n = int(lines[0][0]) if lines else 0
                r.update(n_spheres=n, sphere_radii=[round(float(ln[3]), 3) for ln in lines[1:1 + n]], block_file=os.path.relpath(bf, HERE))
        res[kind] = r
        print(f'  [A] {kind} 1.30: {r}', flush=True)
    return res
...
def summarize(mix, water, zeo, t_start, ffm, wm, final=False):
    ...
    if ok:
        ...
        ms['units_vs_CH3'] = (ms['S_mix_mean'] - REF_CH3_SMIX) / math.hypot(ms['S_mix_mean_err'], REF_CH3_SMIX_ERR)
        ms['(1)_more_than_1.5_above_CH3'] = ms['units_vs_CH3'] > 1.5
    ...
    json.dump({'test': 'MAGI-005 E-24e ② — C₂H₅ S_mix 3씨앗(성분별 막음)', 'assign': 'ASSIGN_MAGI5B_20260925.md §Junseok 16차', 'machine': 'junseok',
               'protocol': 'run_tj1_mix 틀(0.15/0.85 · 1e5 Pa · 298 K · 5,000+15,000) + CO₂ BlockPockets block1.65 · N₂ block1.82',
               'reference': {'CH3_S_mix': [REF_CH3_SMIX, REF_CH3_SMIX_ERR], 'S_Henry_blocked': [SH_BLOCK, ESH_BLOCK], 'S_Henry_unblocked_descriptive': SH_UNBLOCK},
    ...
    for r in (1.30, 1.65):
        ...
        if rows:
            ...
            s['units_vs_parent'] = (ix - REF_PARENT_IDX) / math.hypot(s['index_err'], REF_PARENT_IDX_ERR)
            if r == 1.30:
                s['(2)_more_than_1.5_below_parent'] = s['units_vs_parent'] < -1.5
        wsum[f'block{r:.2f}'] = s
    json.dump({'test': 'MAGI-005 E-24e ③ — C₂H₅ 물 Widom 4씨앗(막음 1.3 = 등록 · 막음 1.65 = 서술)', 'assign': 'ASSIGN_MAGI5B_20260925.md §Junseok 16차',
               ...
               'denominator_KH_CO2_ON_blocked': [KCO2, EKCO2], 'reference_parent_index': [REF_PARENT_IDX, REF_PARENT_IDX_ERR],
               'rows': [water[x] for x in sorted(water)], 'summary': wsum, 'final': final, 'written': time.strftime('%F %T'), 'elapsed_s': round(time.time() - t_start),
               'note': '기계적 사실뿐 — 판정은 종합자. 평균 ± = ±̄/√4, 지수 ± = 두 상대 ± 제곱합, 단위 = d/√(±₁²+±₂²).'},
...
def main():
    ...
    for r in (1.65, 1.82):
        if not os.path.exists(BLOCK[r]):
            ...
            return 2
    print(f'E-24e — CIF {os.path.relpath(CIF, HERE)} · 막음 CO₂ {nsph(1.65)} 구 · N₂ {nsph(1.82)} 구 · 단위셀 {rg.unit_cells(rg.read(CIF))} · 워커 {WORKERS} · '
          f'S_mix 3 + 물 1.3 ×4 + 물 1.65 ×4', flush=True)
    ...
    if n('simulate') not in ('', '0') or n('network') not in ('', '0') or n('lmp_serial') not in ('', '0'):
        ...
        return 1
    for p in (ZBASE, RUNS, OUT_MIX, OUT_W):
        if os.path.exists(p):
            print(f'!! 이전 산출 있음 {p} — 중단', flush=True)
            return 1
    print(f'[A] Zeo++ 물 탐침 1.30 착수 {time.strftime("%T")}', flush=True)
    zeo = zeo_water()
    if not os.path.exists(BLOCK[1.30]) or zeo['block']['rc'] != 0 or zeo['block']['stopped_by_guard']:
        print('!! -block 1.3 실패 — RASPA 안 띄움', flush=True)
        return 1
    if n('network') not in ('', '0'):
        print('!! network 아직 돎 — 중단', flush=True)
        return 1
    os.makedirs(RUNS)
    t0 = time.time() + 2
    jobs = [('mix', (i, k, t0)) for i, k in enumerate((1, 2, 3))]
    jobs += [('water', (3 + i, 1.30, k, t0)) for i, k in enumerate((1, 2, 3, 4))]
    jobs += [('water', (7 + i, 1.65, k, t0)) for i, k in enumerate((1, 2, 3, 4))]
