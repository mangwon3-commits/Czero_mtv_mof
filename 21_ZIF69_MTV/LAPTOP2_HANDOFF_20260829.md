...
Windows 터미널(PowerShell/cmd)에서:
    ...
    cd /d D:\Claude_MOF
    claude --resume ec05659d-160d-40a7-84f8-fa858216547e

- 이 명령은 앱에서 쓰던 **그 세션**(대화·기억 포함)을 CLI 로 엽니다.
  `claude --continue` 도 같은 효과(가장 최근 세션 재개)입니다.
- 반드시 `D:\Claude_MOF` 에서 실행 — 프로젝트 메모리가 이 경로에 묶여 있습니다.
  (WSL 쪽 ~/mof_project 에서 열면 다른 프로젝트로 인식되어 기억이 안 붙습니다.)
- CLI 세션도 지금까지처럼 `MSYS_NO_PATHCONV=1 wsl.exe -e bash -lc '...'` 로
  WSL 의 저장소·계산에 접근합니다.
