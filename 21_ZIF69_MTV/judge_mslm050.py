...
[반드시 함께 적을 것 — 등록문 지시]
    `mslm050` 은 **실현 하나**입니다. 4.6% 는 `mslm` 계열에서 미측정이라
...
N_REF = 6
REL_BATCH = 0.046     # 습윤 WC 상대 배치 산포 (saIm0583 에서 빌려옴)
THRESHOLD = 1.5
...
def main():
    ...
    print(f'\n=== 규약 ⑥-(라) — 깨지는 배치 산포 ===')
    if u >= THRESHOLD:
        print(f'  f = **{f:.1%}**   빌려온 4.6% 의 {f/REL_BATCH:.2f}배')
    ...
    print(f'\n※ mslm050 은 **실현 하나**입니다. 4.6% 는 mslm 계열 미측정이고')
