# -*- coding: utf-8 -*-
"""MAGI-005 E-24g · E-24h (Junseok 실행 사본 — e24g_bp_desktop.py 에서 경로 · 출력 이름만 바꿈 · 분담 변경 12:07 24109aea) 막음 Widom — 닿지 않는 주머니가 나온 새 치환체의 BlockPockets Widom, 전하 ON/OFF.
...
BASE = os.path.join(HERE, 'e24g_bp_runs')
OUT = os.path.join(HERE, 'results_e24g_blockpockets_junseok.json')
...
def block_file(tag, r):
    return os.path.join(HERE, 'e24g_zeo_runs', tag, f'block{r:.2f}', f'{tag}_relaxed.block')
...
def main():
    ...
    if os.path.exists(BASE) or os.path.exists(OUT):
        print('!! 이전 산출(e24g_bp_runs · 결과) 있음 — 중단', flush=True)
        return 1
    res = {}
    ...
    out = {'test': 'MAGI-005 E-24g · E-24h 막음 Widom(BlockPockets) — 주머니 있는 새 치환체, 전하 ON/OFF', 'machine': 'junseok', 'ported_from': 'junseok magi5_e24c_bp_junseok.py (db4346e2)',
           'assign': 'ASSIGN_MAGI5B_20260925.md §HKHOME 21차 · 분담 변경 12:07(24109aea)', 'zeo': 'E-24g -ha -block r 50000 (results_e24g_access_junseok.json)',
