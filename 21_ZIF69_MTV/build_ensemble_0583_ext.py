"""T-C10 — saIm0583 앙상블 **보강** e6~e10 (`build_ensemble_0583.py` 파생, 2026-09-20).

[무엇이 다른가]
    원본은 시드 1..12 를 훑어 5개를 채웁니다(= 기존 e1~e5). 이 파생본은 **시드 6 부터** 시작해
    **새 실현 5개**를 더 만듭니다. 기존 e1~e5 는 시드 범위 밖이라 **아예 방문하지 않습니다**
    — 원본의 "기존 CIF 불가침" 위에 한 겹 더 둔 것입니다. 검사·기각 규칙·인덱스 병합은 원본 그대로.
    시드 하나가 기각되면 다음 시드로 넘어가므로 태그가 e6..e10 이 아닐 수 있습니다(e 뒤 숫자 = 빌더 시드).

    환경변수: ENS_SEED_FROM(기본 6) · ENS_N_TARGET(기본 5) · ENS_MAX_SEED(기본 21)

[왜 — 등록]
    `DESIGN_STUDY_20260903.md` T-C10 "saIm0583 앙상블 보강 e6~e10 (대조군 정밀화)".
    두 문서가 이 계산을 기다리며 문장을 보류하고 있습니다:
      · `LCD_LOADING_CONFOUND.md` L104 — "배치 간 음의 상관을 기제로 읽는 문장은 T-C10(e6~e10) 전까지 쓰지 않습니다."
      · `ASSIGN_20260904.md` L174 — "배치 간 음의 상관(n=5, r=−0.76)은 e6~e10(T-C10) 에서 재검 대상"
    운영 등록·예측은 `TC10_REGISTRATION_20260920.md`.
...
FRAC = 14 / 24.0
WANT = 14
N_TARGET = int(os.environ.get("ENS_N_TARGET", "5"))   # 채택할 새 실현 수
SEED_FROM = int(os.environ.get("ENS_SEED_FROM", "6"))  # T-C10: e1~e5 는 seed 1~5 이므로 6 부터
MAX_SEED = int(os.environ.get("ENS_MAX_SEED", "21"))
...
def main():
    ...
    print(f"  치환 자리 {n_sites}개, 조성 {FRAC*100:.1f}% (자리 {WANT}), "
          f"목표 {N_TARGET}실현, 시드 {SEED_FROM}..{MAX_SEED-1}")
    if a.dry_run:
    ...
    rows, taken = [], []
    for seed in range(SEED_FROM, MAX_SEED):
    ...
    if len(taken) < N_TARGET:
        print(f"  !! 시드 {SEED_FROM}..{MAX_SEED-1} 에서 {len(taken)}개밖에 못 채움. 중단.")
        return 1
