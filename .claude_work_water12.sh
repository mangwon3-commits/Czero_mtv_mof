#!/bin/bash
# 격자 나머지 3종 12작업 — 앙상블 체인이 끝난 뒤에 시작한다.
#
# [왜 기다리나]
...
L=$HOME/.claude_work/water12.log
...
if ! grep -q "ens0583 체인 끝\|!!" "$E" 2>/dev/null; then
  say "!! 24시간 안에 앙상블이 끝나지 않았습니다. 물 12작업을 시작하지 않습니다."
  exit 1
fi
...
say "--- 물 12작업 시작 (물리 ${PHYS}코어, 가용 ${FREE} GB) ---"
WATER_V3_WORKERS="$PHYS" nice -n 5 "$CZ" -u run_water_v3grid_rest3.py >> "$L" 2>&1
rc=$?
say "러너 종료코드 $rc"
if [ -f v3_water_grid/water_results.json ]; then
  ...
  cp v3_water_grid/water_results.json "v3_water_grid/water_results_desktop12.json"
  say "사본 저장: v3_water_grid/water_results_desktop12.json"
fi
say "=== 물 12작업 끝 ==="
